# vmcell — Design Document (v20)

> **v20 (this revision) — builder extraction: bootstrap-in-`vmcell`, in-VM builders in their own
...
- **`vmcell`** — the library (plus the `bench-vm` harness), one package carrying the host feature stack
  ...
  version **independently**, not in lockstep — `vmcell` is at **0.5.0** (bumped from 0.4.0 for the CLI
  extraction and the newly-`pub` builder-reuse surface) and `vmcell-protocol` at 0.3.0 (the additive
  `Resync`/`ResyncAck` variants and the `AgentClient` `Timeouts`-param arity change drove those bumps); the
  binary members version lower.
...
extraction plus the newly-`pub` reuse surface (a `cargo semver-checks` event on the PR).
