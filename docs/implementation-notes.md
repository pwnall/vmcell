...
- **`RtNetlink` TAP Persistence:** A `tun_tap::Iface` created inside a network namespace will be immediately destroyed by the Linux kernel if the Rust `Iface` object is dropped. To fix `NetNamespace` and `RtNetlink`, we had to alter the `Netlink` trait's `setup_tap` method to return an `Option<tun_tap::Iface>` so the orchestrator could keep the interface alive for the lifetime of the VM.
- **Doctest Asynchronous Execution:** Evaluating `async` logic in `cargo test --doc` examples resulted in missing dependency compilation errors because the required executor features weren't available. This was resolved by explicitly adding `tokio` (with `macros`, `rt-multi-thread`, `net`, and `time` features) to the `[dev-dependencies]` in `Cargo.toml`. The examples are now seamlessly compiled and verified as fully active doctests.
