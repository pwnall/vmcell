...
- **Metrics module is a stub:** The `src/metrics.rs` file defines only `ResourceUsage` and the cgroup/procfs read logic lives inline in the orchestrator. The injected `CgroupFs` seam and the full cgroup slice lifecycle are not yet extracted into a dedicated module. This is a refactoring gap, not a functional regression for the currently exercised paths.
