...
- **`StageOutputs` struct missing fields**: Added the `artifacts` field to `StageOutputs` but failed to update instances where `StageOutputs {}` was instantiated across `kernel.rs`, `rootfs.rs`, and `snapshot.rs`. Had to refactor to use `StageOutputs::default()` to fix the missing field errors.
- **FakeVmm Lifecycle Calls Check**: `TestVm::start()` automatically calls `boot()` immediately after `create()`. The `FakeVmInstance` captures both operations in its `calls` vector. Verifying that the creation step was executed properly required asserting a length of `2` rather than `1` and checking that both `create` and `boot` were successfully tracked.
- **Duplicate `[dev-dependencies]` and Test Modules**: Automated editing macros caused multiple `mod tests { ... }` blocks and duplicate `[dev-dependencies]` keys in `Cargo.toml`. Merging the dependencies and test cases into single unified blocks resolved the compilation errors.
...
- **Property Testing (`proptest`):** We added `proptest` to `dev-dependencies` to fuzz inputs like `VmConfigBuilder` paths and subnet configurations (`/30` IPs). However, deep property tests for the `AgentProtocol` framing and serialization layer have not yet been fully realized.
