...
Suggested addition to docs/implementation-notes.md, appended to the "Tier E3+E4 landed (daemon pause/resume routes; streaming artifact upload)" entry (E3 half):

**E3's live test shipped with a premise that could not hold, and it is the reusable lesson.** The route was right; `pause_and_resume_over_rest_stop_and_restart_the_guest_clock` was not, and it had never been executed (the blessed runner was stale when it was written). It read the guest's `/proc/uptime` across a paused window and asserted it stood still. It never can: a KVM guest's timebase is the HOST's. Our guests select `tsc` as their clocksource — the raw host TSC, which counts on whether or not a vCPU is scheduled — and the alternative, `kvm-clock`, is a pvclock structure derived from that same host TSC, so neither stops when `vm.pause` stops the vCPUs. Measured on this tree, with an in-guest spin loop appending one byte per iteration and the VM's own `cloud-hypervisor` `vcpu*` threads sampled out of `/proc`: across a 3.02 s pause the guest's uptime advanced 3.02 s (the whole window) while the vCPU thread accumulated 0 ticks of utime+stime and the loop completed 1,946 iterations against 1,284,325 over an identical un-paused control — 0.15 %, all of it in the un-paused edges around the pause/resume calls. The assertion was therefore unfalsifiable-by-construction in one direction and false in the other: it would have "passed" a pause that never reached the VMM at all, had the numbers fallen the other way. This is the same shape as the v33 guest-tuning finding (a channel whose two ends default to the same value is unfalsifiable) with the roles of the two ends played by the guest's clock and the host's: a guest clock is a proxy signal for "the guest ran", and asserting on it is the proxy-signal habit AGENTS.md bans, one level subtler than `contains("html")`.
