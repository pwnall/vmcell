...
| [52] **Toolchain honesty**: `rust-toolchain.toml` is the single toolchain source; `rust-version` lives once in `[workspace.package]` and **equals the tested floor (1.88)**, sync-asserted in `ci` | declared-vs-effective MSRV drift — an understated `rust-version` lets an MSRV-aware resolver hand a 1.85 consumer the *vulnerable* `time 0.3.45` the lockfile exists to avoid |
