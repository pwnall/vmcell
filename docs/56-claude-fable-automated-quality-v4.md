...
```toml
# Declared MSRV = the tested floor. v2 bumps this from 1.85: the committed lockfile needs 1.88
# (time 0.3.47 pins RUSTSEC-2026-0009), and an UNDERSTATED rust-version is worse than cosmetic —
# an MSRV-aware resolver serving a 1.85 consumer resolves time back to the vulnerable 0.3.45 the
# lockfile exists to avoid. Kept in lockstep with rust-toolchain.toml + [workspace.package]
# rust-version by the sync assertion in `ci`.
msrv = "1.88"
...
## `scripts/` (ban scripts, preflight) — delta: none

The repo's `ban-global-state.sh` + `test-ban-global-state.sh` self-test satisfy meta-rule 2 in both
directions, and `ban-legacy-terms` / `ban-agent-ip-shellout` cover their invariants; the richer
`review-preflight-priv.sh` stands. All of them are now inside the `shellcheck` gate.
...
| Part E Phase-0 preflight | `scripts/review-preflight-priv.sh` |
