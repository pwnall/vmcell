# vmcell — Design Document (v27)

> **v27 (this revision) — full integration.** This document folds the v23 unified design and the three
> focused amendments that followed it — **v24** (privileged-window hardening, `docs/60`), **v25** (the
> OverlayStore seam + fork/branch lineage, `docs/61`), and **v26** (persistent interactive sessions,
> `docs/62`) — together with the settled deviations recorded in `docs/implementation-notes.md`, into a
> single self-contained design. Nothing is dropped and the base architecture is unchanged; each amendment's
> surgical base edits are applied in place, its new component is integrated as a full section, and its
> "Amendments to the base document" list is consumed (it becomes those edits, not a leftover list). The
> three new components are **Part VII / §20** (shrinking and confining the privileged window — VMM seccomp,
> a jailer-equivalent, and the setup broker that ships as the default for `vmcelld`), **Part VIII / §21**
> (the injectable `OverlayStore` seam and the `Lineage` fork/branch handle over the existing zygote), and
> **Part IX / §22** (PTY, streaming stdin, and multiplexed exec as an additive session layer over the vsock
> control plane). The cross-cutting invariants grow from §12.20 to **§12.29**; §14 grows the matching gates;
> §16/§17 graduate all three roadmap items to built; and the version register is `vmcell` **0.9.0**,
> `vmcell-protocol` **0.4.0**, `vmcell-privilege` **0.2.0**, `vmcell-broker` from **0.1.0**, with
> `vmcell-guest-agent` at **0.3.0** — every step additive-or-0.x-minor and `cargo semver-checks`-clean. The
> per-amendment summaries below record what each layer added and where.
>
> **v26 — persistent interactive sessions (§22).** Beside the one-shot `exec`, an additive **session** layer
> over the vsock control plane: a **PTY** session (controlling-terminal via `setsid`+`TIOCSCTTY`, `isatty`
> true, resizable with `Winsize`→`SIGWINCH`) or a **pipe** session, **streaming stdin** over the session's
> life (`Stdin`/`StdinEof`), and **multiplexed** concurrent sessions over one connection keyed by
> `SessionId`. Realized as **eight append-only channelized `Message` variants** (indices 8–15, discriminant-
> pinned) plus `SessionId`/`SessionSpec`/`PtyConfig`; a guest **non-blocking dispatch loop** with a **single
> per-connection writer** and **connection-owns-its-sessions** teardown; and a host `agent::session`
> multiplexer beside `AgentClient` sharing the one connect/handshake helper. Adds invariants §12.26–§12.29.
> Library + a minimal CLI (`run --stdin`/`--tty`) ship and are KVM-validated (`tests/session.rs`, 14/14 on
> CH+FC+QEMU); daemon streaming sessions are §22.7 forward work. `vmcell-protocol` 0.3.0 → 0.4.0, `vmcell`
> 0.8.0 → 0.9.0, `vmcell-guest-agent` 0.2.0 → 0.3.0.
>
> **v25 — the OverlayStore seam + fork/branch lineage (§21).** The zygote fan-out's per-clone copy-on-write
> copy (§9.4) is lifted behind an injectable **`overlay::OverlayStore`** trait (`ReflinkOverlayStore` prod
> impl + a recording double, the `CgroupFs`-seam pattern), and a **`lineage::Lineage`** handle adds `fork`
> (mint a live child VM at a node) and `branch` (freeze a running descendant into a **new** node whose
> parent is this one) over the existing `Zygote` — an immutable, acyclic tree of provenance, each node a
> flat self-contained single snapshot (no backing chain, §21.3). No new clone mechanics: it reuses `Zygote`,
> `mac_math`/`ip_math`, and `check_clone_eligible` (one law). Adds invariants §12.24–§12.25. Built, gated,
> and KVM-validated (`tests/lineage.rs` on CH+FC). `vmcell` 0.7.0 → 0.8.0 (`restore_cow` gains the
> `Arc<dyn OverlayStore>` seam parameter — a 0.x minor, semver-checks-clean).
>
> **v24 — privileged-window hardening (§20).** Three layers confine the caps the privileged mode and
> `vmcelld` hold (§12.14): **Layer 1** — every VMM spawns under its own seccomp filter, selected by **one
> predicate** `vmm_seccomp_args` (CH `--seccomp true`, FC's built-in, QEMU `-sandbox on,…` — closing the
> hole where QEMU ran unconfined), failing loud (`Error::Unsupported`) on a policy a backend can't honor;
> **Layer 2** — a jailer-equivalent pre-exec hardening of the VMM child (`no_new_privs` + `RLIMIT_CORE=0` +
> `PR_SET_DUMPABLE=0`, an opt-in ambient-cap clear, and an optional coarse seccompiler deny-list), a pure
> `JailSpec` with the async-signal-safe `apply_jail` as its only impure edge; **Layer 3** — the **setup
> broker**, now the **default** for `vmcelld`: the daemon forks, the broker child keeps the three caps and
> owns the VM `Registry`, and the parent **drops all caps** and serves HTTP, forwarding VM ops over the
> multiplexed `VmEngine` RPC (§12.23 — the network surface never holds the caps). Seccomp uses **seccompiler**
> (Apache-2.0/BSD-3, the rust-vmm library CH/FC use); the LGPL `libseccomp` family is **banned by name** in
> `deny.toml` (the license gate cannot see a C library linked through `build.rs`). Adds invariants
> §12.21–§12.23. Layers 1–2 and the broker cutover are KVM-validated (`just test-privileged`;
> `just test-daemon` 12/12 with the parent holding no caps). `vmcell` 0.6.0 → 0.7.0, `vmcell-privilege`
> 0.1.0 → 0.2.0, `vmcell-broker` from 0.1.0.
>
> **v23 — unification.** This document folds the two focused amendments that followed
...
**How to read this document.** It is written for someone learning the project, in nine parts:
...
- **Part VII — Privileged-window hardening** (§20): the three layers that confine the capabilities the
  privileged mode and the daemon hold — each VMM's own seccomp filter, a jailer-equivalent pre-exec
  hardening of the VMM child, and the setup broker that keeps the caps off the daemon's network surface.
- **Part VIII — Copy-on-write clone as a seam, and fork/branch lineage** (§21): the injectable
  `OverlayStore` seam behind the zygote's per-clone CoW copy, and the `Lineage` handle that adds
  `fork`/`branch` over the existing fan-out.
- **Part IX — Persistent interactive sessions** (§22): the additive session layer over the vsock control
  plane — PTY (controlling-terminal) or pipe sessions, streaming stdin, and multiplexed concurrent exec
  keyed by `SessionId` — beside the untouched one-shot `exec`.
...
| **Privileged-window hardening** | Three layers (§20): every VMM under its own seccomp (`vmm_seccomp_args`, one law), a jailer-equivalent pre-exec hardening (`no_new_privs` + rlimits + optional ambient-clear + optional seccompiler deny-list), and a **setup broker** — a minimal privileged `vmcell-broker` child that holds the caps and owns the VM registry, so the cap-dropped `vmcelld` parent serves HTTP with **no capabilities** (§12.21–§12.23). Seccomp uses **seccompiler** (Apache-2.0/BSD-3, the rust-vmm library CH/FC use); the LGPL `libseccomp` family is banned by name in `deny.toml`. |
| **CoW clone as a seam + lineage** | The zygote CoW copy (§9.4) is lifted behind an injectable **`OverlayStore`** (trait + `ReflinkOverlayStore` + recording double, the `CgroupFs` pattern), and a **`Lineage`** handle adds `fork` (mint a live child) / `branch` (freeze a running descendant into a new node) over the existing `Zygote`, with immutable, acyclic lineage (§12.24–§12.25). Each node is a flat single snapshot (no backing chain, §21.3). |
| **Interactive sessions** | Beside the one-shot `exec` (§4), an additive **session** layer (§22): a **PTY** (controlling-terminal, `isatty` true, resizable) or **pipe** session, **streaming stdin** over the session's life, and **multiplexed** concurrent sessions over one connection keyed by `SessionId` — eight append-only channelized `Message` variants, a guest non-blocking dispatch loop with one per-connection writer and connection-owns-its-sessions teardown (§12.26–§12.29), and a host `agent::session` multiplexer. Library + CLI ship; daemon streaming is §22.7 forward work. |
...
#[non_exhaustive]
pub enum Message {
    // indices 0–7 — the one-shot control plane, unchanged byte-for-byte:
    ...
    // indices 8–15 — the append-only session layer (§22), each frame keyed by SessionId:
...
pub struct SessionSpec { pub command: ExecRequest, pub pty: Option<PtyConfig> } // reuses ExecRequest (§22.2.1)
```
...
The eight `Session*` variants (indices 8–15) and the `SessionId`/`SessionSpec`/`PtyConfig` value types are
the **append-only** session layer added for persistent interactive sessions (PTY / streaming stdin /
multiplexed exec, §22). `postcard` encodes a variant by its zero-based declaration index, so the order *is*
the wire format: the session variants are **appended** (never reordered/removed) after `ResyncAck`, the
one-shot indices 0–7 keep their bytes exactly, and a KVM-free **discriminant-stability** test pins each
appended variant to its index (§22.2). The one-shot `Exec` deliberately stays **id-less** — a host that
wants multiplexing uses the session API on a *separate* connection (§22.4), so the heavily-tested one-shot
...
**Beside `AgentClient`: the `agent::session` multiplexer (§22).** For persistent interactive sessions the
host gains `vmcell::agent::session` (`SessionMux`/`Session`/`SessionEvent`), a multiplexer that owns **its
own** vsock connection so it never shares a stream with — or interleaves one-shot and session frames
against — the cached `AgentClient`. It reuses the **one** connect/handshake helper `AgentClient` already
uses (the byte-by-byte `OK` line + `Ready`, §12.5), now refactored into a shared `connect_framed(...)` so
the fragile handshake has exactly one implementation. Its full shape — a single background reader task, an
`mpsc`-per-session demux, and one `Arc<Mutex<SplitSink>>` writer mirroring the guest's single-writer
discipline — is §22.4.
...
- **Mount** `proc`, `sys`, `devtmpfs`, **`devpts` at `/dev/pts`** (best-effort, right after `devtmpfs` — it
  is *not* in the fatal core-mount set `{overlay, /proc, /dev}`, so a failed mount fails only PTY sessions,
  never the control plane, §22.3), the virtio-fs tags, and set up the **tmpfs `overlayfs`** over the
...
- **Dispatch each connection non-blocking, through one writer, owning its sessions (§22.3, §12.26–§12.29).**
  `serve_connection` splits the accepted stream into a read half (the dispatch loop) and a `try_clone`d
  write half behind an `Arc<Mutex<VsockStream>>` — the **single** per-connection writer every frame goes
  through (the initial `Ready`, one-shot output, put-file/resync acks, and all session pump output), so
  concurrent session frames never interleave-corrupt on the wire (§12.28). The loop reads a frame and
  dispatches without ever blocking on a child: `Exec`/`PutFile`/`Resync` run the existing handlers
  (unchanged behavior, now writing through the shared writer; one-shot `Exec` is still synchronous), while
  `OpenSession` spawns a PTY (`setsid`+`TIOCSCTTY` on a `/dev/pts` slave, `isatty` true, resizable) or pipe
  session, registers a `SessionHandle` in the per-connection `SessionId → SessionHandle` table, and returns
  immediately. `Stdin`/`StdinEof`/`Winsize`/`CloseSession` act on the looked-up handle. When the loop ends
  for any reason (disconnect, transport error, desync), the connection **kills every still-open session's
  process group and closes its fds before returning** — no interactive session outlives its connection
  (§12.27). The full session mechanics are §22.3.
...
2. **Copy-on-write per clone, through the injected `OverlayStore` seam (§21.2).** To mint a clone, **CoW-copy
   the whole suspend dir** into the clone's own scratch dir, then `restore()` + `resume()` from that private
   copy. The copy is materialized through the injectable `overlay::OverlayStore::clone_tree(master,
   private_dst)` seam (§21) — the production `ReflinkOverlayStore` wraps the same `reflink.rs` primitive, so
   on a reflink-capable host filesystem (XFS, Btrfs, bcachefs) the copy is a near-instant block-level
   `FICLONE` that shares physical storage with the master until a clone writes, and on any other filesystem
   (ext4, tmpfs) it degrades to a full byte copy — correct, just not free. The copy is reported as
   `CowSupport::{Reflink, FullCopy}` so a caller building a large pool on a non-reflink filesystem can warn
   (or pick a different scratch dir). The vetted `reflink-copy` crate owns the ioctl and the fallback, so no
   new `unsafe` enters the tree; lifting it behind the seam makes the clone step unit-testable with **no
   reflink filesystem** and admits a future backing store without touching the orchestrator (§12.24).
...
**A lineage layer over the flat fan-out (§21.4).** The fan-out above is *flat*: one immutable master, many
independent clones, no recorded parent→child relationship. The **`Lineage`** handle (§21) adds a *tree of
provenance* on top of `Zygote` without a second copy of the clone logic — `fork` mints a live child VM at a
node (delegating to `spawn_clone`), and `branch` freezes a *running* descendant that has done work into a
**new** node whose parent is the one it was forked from, so `root → b1 → b2` is a self-contained chain of
single-snapshot zygotes. Each node reuses this section's CoW/fan-out mechanics, the `restore_rotates_host_paths`
concurrent-fan-out gate, and the §12.1 eligibility law unchanged; the lineage adds only immutable ancestry
metadata (§12.25). The mechanism is §21.
...
- **`vmcell`** — the library (plus the `bench-vm` harness), one package carrying the host feature stack
  ...
  version **independently**, not in lockstep — `vmcell` is at **0.9.0** (0.4.0→0.5.0 for the CLI
  extraction and the newly-`pub` builder-reuse surface, 0.5.0→0.6.0 for the `resource_prefix` /
  `vmcell::naming` surface the daemon needs, then 0.6.0→0.7.0 for the §20 `vmm_seccomp`/`jail` surface,
  0.7.0→0.8.0 for the §21 `OverlayStore`-seam `restore_cow` parameter, and 0.8.0→0.9.0 for the §22
  `agent::session` surface, §10.2/§18.4) and `vmcell-protocol` at **0.4.0** (the additive `Resync`/`ResyncAck`
  variants and the `AgentClient` `Timeouts`-param arity change, then the eight appended `Session*` variants of
  §22.2); `vmcell-test-runner` is at **0.3.0** (the `vmcell-privilege` extraction, §18.2); the daemon/privilege
  members (`vmcell-daemon`, `vmcelld`, `vmcell-daemon-client`, `vmcelld-ctl`, §18) version from **0.1.0**,
  except `vmcell-privilege` at **0.2.0** (the §20 broker parent cap-drop plan) and the new `vmcell-broker`
  from **0.1.0** (§20.5); the remaining binary members version lower.
...
- **`vmcell-broker`** — the lean **privileged spawn helper** + `BrokerClient` (§20.5). It holds the three
  caps and performs VM setup/spawn/teardown/sweep on behalf of a cap-dropped `vmcelld` parent, links
  `vmcell`'s **net-privileged/metrics subset + `vmcell-privilege`** (never the daemon's axum/hyper web
  stack — a lean-tree assertion pins that `vmcell-daemon`/axum are absent), and versions from **0.1.0**. The
  jailer-equivalent it applies lives in `vmcell::vmm::jail` (not here), so `vmcell-broker` reuses the same
  `JailSpec`/`apply_jail` the in-process spawn uses (one law, §12.22).
...
The six daemon/privilege members form a **second** acyclic star on top of `vmcell` — the daemon depends
on `vmcell` (never the reverse), and the client links only the daemon's DTOs — with `vmcell-privilege`
sitting in the lean tier beside `vmcell-test-runner`, and **`vmcell-broker`** linking `vmcell`'s
net-privileged/metrics subset + `vmcell-privilege` but **never** `vmcell-daemon`'s web stack (its own
lean-tree assertion, §20.5); the full graph is §18.1. `vmcell-broker`'s privileged spawn reuses
`vmcell::vmm::jail`, so the jailer-equivalent stays out of the lean `vmcell-privilege`/`vmcell-test-runner`
tier (which never spawns a VMM), keeping those crates' lean-tree assertions green — the one recorded
placement deviation (implementation-notes v24(a)).
...
vmm/seccomp.rs   # vmm_seccomp_args: the ONE (backend, VmmSeccomp)→CLI-flag predicate (§20.3, §12.21)
vmm/jail.rs      # JailSpec + async-signal-safe apply_jail: the jailer-equivalent, seccompiler deny-list (§20.4, §12.22)
agent/           # AgentClient (host vsock client, handshake + desync); agent::session multiplexer (§22.4); re-exports the protocol wire types
...
reflink.rs       # the FICLONE-or-copy primitive behind ReflinkOverlayStore (§9.4, §21.2, forbid(unsafe))
overlay.rs       # OverlayStore seam: trait + ReflinkOverlayStore + RecordingOverlayStore; one clone-materialization law (§21.2, §12.24)
zygote.rs        # Zygote: suspend once, mint many; the concurrent-fan-out gate (§9.4)
lineage.rs       # Lineage/LineageId/LineageAllocator: fork/branch over Zygote; immutable acyclic provenance (§21.4, §12.25)
...
pub struct VmConfig {
    ...
    pub vmm_seccomp: VmmSeccomp,        // the VMM subprocess's OWN seccomp filter: Enforcing (default) | Log | Disabled (§20.3, §12.21)
    pub jail: JailConfig,               // the jailer-equivalent pre-exec hardening of the VMM child; default hardened() (§20.4, §12.22)
}
...
impl<V: Vmm> MicroVm<V> {
    ...
    pub async fn restore_cow(vmm: &V, zygote_dir: &Path, cfg: VmConfig, cids: Arc<CidAllocator>, vmids: VmidAllocator, cgroups: Box<dyn CgroupFs>, overlay_store: Arc<dyn OverlayStore>) -> Result<(Self, CowSupport)>; // CoW-copy the suspend dir through the injected seam first (§9.4, §21.2)
...
impl Zygote {
    ...
    pub fn with_overlay_store(self, store: Arc<dyn OverlayStore>) -> Self; // inject the CoW seam; default ReflinkOverlayStore (§21.2)
...
// ---- overlay.rs — the injectable copy-on-write clone seam (§21.2) ----
pub trait OverlayStore: Send + Sync + std::fmt::Debug {
    fn clone_tree(&self, src: &Path, dst: &Path) -> Result<CowSupport>; // CoW-clone a snapshot dir into a fresh private copy; dst must not exist
    fn probe(&self, dir: &Path) -> CowSupport;                          // side-effect-free reflink-support probe
}
#[derive(Clone, Copy, Debug, Default)]
pub struct ReflinkOverlayStore;   // production: FICLONE where supported, full byte copy otherwise (wraps reflink.rs)
// RecordingOverlayStore (test double) records every (src, dst) and returns a configurable CowSupport.

// ---- lineage.rs — fork/branch over Zygote, immutable acyclic provenance (§21.4) ----
...
impl Lineage {
    pub async fn fork_from_vm<V: Vmm>(vm: &mut MicroVm<V>, cfg: VmConfig, dir: impl Into<PathBuf>, allocator: LineageAllocator, store: Arc<dyn OverlayStore>) -> Result<Self>; // root a lineage (gen 0)
    pub async fn from_snapshot_dir(dir: impl Into<PathBuf>, cfg: VmConfig, allocator: LineageAllocator, store: Arc<dyn OverlayStore>) -> Result<Self>;                        // adopt a SnapshotStage artifact as root
    pub fn id(&self) -> LineageId;
    pub fn parent(&self) -> Option<LineageId>;
    pub fn generation(&self) -> u32;
    pub fn ancestry(&self) -> &[LineageId];             // root .. parent inclusive
    pub fn is_ancestor_of(&self, other: &Lineage) -> bool; // allocator-scoped (Arc::ptr_eq) then id-in-ancestry
    pub fn master_dir(&self) -> &Path;
    pub fn probe_cow_support(&self) -> CowSupport;
    pub async fn fork<V: Vmm>(&self, vmm: &V, cids: Arc<CidAllocator>, vmids: VmidAllocator, cgroups: Box<dyn CgroupFs>) -> Result<MicroVm<V>>;      // mint ONE live child at this node
    pub async fn fork_many<V, F>(&self, vmm: &V, count: usize, cids: Arc<CidAllocator>, vmids: VmidAllocator, make_cgroups: F) -> Result<Vec<MicroVm<V>>> where /* … */; // concurrent fan-out (reuses the §9.4 gate)
    pub async fn branch<V: Vmm>(&self, child: &mut MicroVm<V>, dir: impl Into<PathBuf>) -> Result<Lineage>; // freeze a running descendant into a NEW node (gen+1); re-checks eligibility (§12.1)
}

// ---- agent::session — the host multiplexer, re-exported at vmcell::agent (§22.4) ----
pub struct SessionMux { /* own vsock connection; a demux registry; a reader task; one writer sink; next-id */ }
...
// ---- vmm::seccomp / config — the VMM's own seccomp policy + the jailer-equivalent config (§20) ----
pub enum VmmSeccomp { Enforcing, Log, Disabled }  // default Enforcing; Disabled is a logged, explicit opt-out
pub fn vmm_seccomp_args(backend: &str, policy: VmmSeccomp) -> Result<Vec<String>>; // the ONE (backend,policy)→flags law; Unsupported on an impossible combo (§12.21)
pub struct JailConfig { /* no_new_privs, clear_ambient_caps, non_dumpable, rlimit_core/fsize/nofile, seccomp_deny_list: bool */ }
impl JailConfig { pub fn hardened() -> Self; } // default: no_new_privs + RLIMIT_CORE=0 + non_dumpable on; clear_ambient_caps + seccomp_deny_list OFF (§20.4, §20.9)
```
...
ergonomics; the CoW copy is materialized through the injected `OverlayStore` seam (below), whose production
`ReflinkOverlayStore` wraps `reflink.rs` (`#![forbid(unsafe_code)]` — the `FICLONE` ioctl and its full-copy
fallback are the vetted `reflink-copy` crate's, so no `unsafe` enters the tree).

**Three additive surfaces layered by the v24–v26 components, driven from §20/§21/§22.** `VmConfig` gains
`vmm_seccomp` (default `Enforcing`) and `jail` (default `JailConfig::hardened()`) — the VMM's own seccomp
policy and the jailer-equivalent applied to the spawned VMM child (§20); both additive. The
**`overlay::OverlayStore`** seam (trait + `ReflinkOverlayStore` + a recording double) is the one
clone-materialization law `restore_cow`/`Zygote`/`Lineage` all thread a store through — injectable exactly
like `CgroupFs`, defaulting to `ReflinkOverlayStore`, so the CoW step is unit-testable with no reflink
filesystem (§21.2). The **`lineage::Lineage`** handle (with `LineageId`/`LineageAllocator`) adds
`fork`/`fork_many`/`branch`/`fork_from_vm` over `Zygote`, carrying immutable, acyclic provenance and
inventing no new identity or eligibility logic (§21.4–§21.5). The **`agent::session`** multiplexer
(`SessionMux`/`Session`/`SessionEvent`/`SessionSpecBuilder`), plus `MicroVm::connect_sessions` and the
`vmcell-protocol` `SessionId`/`SessionSpec`/`PtyConfig` re-exports, are the host surface for persistent
interactive sessions (§22.4); no existing signature changes (`exec`/`put_file`/`resync`/`agent` are
untouched). Version register: `restore_cow`'s new `Arc<dyn OverlayStore>` parameter is the sole breaking
change, a 0.x-minor bump; everything else is additive, and the whole surface is `cargo semver-checks`-clean.
...
`restore_rotates_host_paths`, §3.3 / §9.4.) This immutability **extends to every `Lineage` branch node**
(§21): each node is a complete, immutable suspend image its descendants CoW-clone from, so the tree never
mutates a node under a live descendant (§12.25).
...
### 12.14 The cap-holder retains caps; it never drops-and-execs, and never runs degraded

The cap-holding process is a *long-lived* server that must itself perform privileged VM operations
(netns/tap/nft) for the whole process lifetime, so — unlike the transient test-runner, which raises ambient
caps and execs — it runs the blessing **precondition** (the three caps present in the **effective** set, or
`euid == 0`) and then **keeps** them: no uid drop, no ambient raise, no bounding-set shrink, no `exec`
(§18.2). If the precondition fails it prints the `setcap …+ep` remediation and **refuses to start** — never
a daemon that came up without `CAP_NET_ADMIN` and fails every privileged create at first use
(fail-loud-at-construction). Owner: `vmcelld` main + `vmcell-privilege::ensure_blessed_or_explain`. Gate:
the moved `compute_missing` effective-vs-permitted test.

**Which process holds the caps depends on the broker (§20.5).** By **default** `vmcelld` forks the setup
broker: the **broker child** is this cap-holder — it retains the three caps and owns the VM `Registry`,
while the HTTP-serving parent drops all caps (§12.23). This invariant then governs the broker child. Under
`--no-setup-broker` there is no fork and the single `vmcelld` process is the cap-holder — the original
retain-caps form, now the **fallback**, not the default. Either way the cap-holder retains rather than
drops-and-execs; §12.23 additionally requires that whoever holds the caps not be the process parsing network
input.
...
The next nine invariants are introduced by the privileged-window hardening (§20), the OverlayStore
seam + lineage (§21), and the interactive-session layer (§22). They are laws in the same register — one
predicate, one owner, a gate that can go red.

### 12.21 Every backend spawns under its own seccomp; one predicate decides the flag

> **No backend is launched without consulting `vmm_seccomp_args`, the single place a `VmmSeccomp` policy
> becomes a backend flag, which returns `Error::Unsupported` (never a silent downgrade) for a policy a
> backend cannot honor.**

The predicate maps `(backend, VmmSeccomp)` to the CLI tokens — CH `--seccomp true|log|false`, FC's built-in
filter (`Enforcing → []`, `Disabled → --no-seccomp`), QEMU `-sandbox on,obsolete=deny,…` — and returns a
typed `Unsupported` for the two impossible combos (`Log` on FC/QEMU) rather than degrading, the §7.2
capability-honesty contract applied to seccomp. This closes a real hole: QEMU was spawned with **no**
`-sandbox`, unconfined (§20.3). Owner: `vmm::seccomp::vmm_seccomp_args`; consumers are `spawn_ch`/`spawn_fc`/
the QEMU cmd builder. Gate: the golden-args one-law test (red on a new backend that skips the predicate or
emits a wrong flag) + the two `Unsupported`-combo tests — the same discipline as `mac_math` and
`config_has_vhost_user_device`.

### 12.22 One jail, applied by every VMM spawn; the pure spec is separate from the syscall edge

> **The VMM child is hardened by exactly one `JailSpec` through exactly one `apply_jail`, called from both
> the in-process `build_vmm_cmd` and the broker's `SpawnVmm` — never a second copy.**

The spec is pure data (`no_new_privs`, `RLIMIT_CORE=0`, `PR_SET_DUMPABLE=0` on by default; `clear_ambient_caps`
+ the optional seccompiler deny-list off by default, §20.4); the async-signal-safe `apply_jail` is the only
impure edge, its `SAFETY` comments proving async-signal-safety. The apply order is load-bearing (rlimits →
dumpable → ambient → `no_new_privs` → seccomp → execve, since `SECCOMP_SET_MODE_FILTER` needs `no_new_privs`
first). Owner: `vmcell::vmm::jail`. Gate: the `/proc/self/status` stand-in test (`NoNewPrivs:1`, `CapEff:0`,
zero core limit) + the deny-list `mount→EPERM`/`write→ok` test — both root-free and KVM-free, both red on the
inverse.

### 12.23 In the broker model, the network surface never holds the caps; the cap-holder never parses network input

> **The `vmcelld` parent drops all three caps before the HTTP listener binds; the broker child serves only
> the typed, bounded, fd-passing IPC menu — no network.**

The parent's cap-drop is a pure `plan_broker_parent_drop` (unit-tested against its inverses) applied before
the listener binds; the broker child validates every request field at the boundary — honor or reject, no
silent default (§12.2) — which *is* the broker's security value. A memory-safety or logic bug reachable
through the HTTP request parser can no longer reach the caps. Owner: `vmcell-broker` + `vmcelld` main. Gate:
the priv-drop-plan inverse tests (parent ends with an empty effective/permitted/ambient set + `no_new_privs`,
preserves the `kvm` gid), the framed-codec round-trip + over-cap-reject test, and the dispatch-against-fakes
call-order/residue-gone tests; the live cutover is the §20.9 KVM step (`just test-daemon` 12/12 with the
parent holding no usable caps).

### 12.24 Every CoW clone is materialized through the injected `OverlayStore`, into a private copy, never the master

> **No restore path copies a suspend directory except by calling `OverlayStore::clone_tree(master,
> private_dst)`, where `private_dst` lives inside the clone's own scratch dir and is distinct from the
> master.**

The seam is the single clone-materialization law; the reflink `FICLONE` ioctl and its full-copy fallback stay
inside `reflink.rs` (one impl, wrapped by `ReflinkOverlayStore`), so `#![forbid(unsafe_code)]` still holds
across the tree. The private `dst` is reclaimed by the §12.10 teardown. Owner: `overlay::OverlayStore` +
`MicroVm::restore_inner`. Gate: the `RecordingOverlayStore` test asserting the restore path calls
`clone_tree` once per clone with a private, non-master `dst` (red if a restore is handed the master); the
`ReflinkOverlayStore` faithful-and-independent-copy test (writing the clone never mutates the master); the
probe-agrees-with-actual-clone test.

### 12.25 A lineage is immutable and acyclic; a branch's ancestry is its parent's plus its parent, generation strictly increases, and no node is its own ancestor

> **`branch` produces a node with `parent = Some(self.id)`, `generation = self.generation + 1`, and
> `ancestry = self.ancestry ++ [self.id]`; a root has `parent = None`, `generation = 0`, empty ancestry.**

`is_ancestor_of` is antisymmetric and transitive by construction, and **cross-family-safe**: it first checks
the two nodes share a `LineageAllocator` (`Arc::ptr_eq`), then that `self.id` is in `other`'s ancestry — so
two nodes minted by distinct allocators are never a false-positive ancestry even when their ids collide (each
allocator starts at `L1`). Each node's snapshot is an immutable master (§12.12 extends to branch nodes), so
the tree never mutates a node under a live descendant. Owner: `lineage::Lineage`. Gate: the
ancestry-construction unit test (root → b1 → b2 generations/parents/ancestries), the `is_ancestor_of`
antisymmetry test, the no-self-ancestor test, and the `LineageAllocator` distinct-ids test — each red on its
inverse.

### 12.26 Session I/O is channelized and terminally-framed

> **Every session data/control frame carries its `SessionId`; a session's lifecycle on the wire is
> zero-or-more `SessionStdout`/`SessionStderr` frames then exactly one terminal `SessionExit`, the last
> frame for that id.**

The guest never emits a legacy id-less `Stdout`/`Stderr`/`Exit` for a session, never a second `SessionExit`,
and reports a failed open as `SessionStderr` + `SessionExit(127)` (the one-shot spawn-failure convention, so
there is one terminal-frame convention and no separate error variant). Owner: guest `run_session` + host
demux. Gate: the host demux multiplex test (interleaved ids delivered to the right handle, each stream ending
at its `SessionExit`, a stray frame after exit dropped not delivered) + the guest spawn-failure →
`SessionStderr`+`SessionExit(127)` unit test — each red on its inverse.

### 12.27 A connection owns its sessions

> **When a connection's dispatch loop ends (host disconnect, transport error, or desync), every still-open
> session's process group is `SIGKILL`ed and its fds are closed before the connection thread returns.**

No session outlives its connection, and dropping the host `SessionMux` is a sufficient teardown trigger — so
a host that forgets to `close()` still cannot leak guest processes. Sessions do not survive snapshot/restore
either: a restored VM re-binds the listener and the host reconnects on a fresh connection (the "persistent"
in the feature name is *within a session's life across many frames*, not across a VM restore). Owner: guest
`serve_connection` teardown. Gate: the KVM live connection-drop residue test — open a persistent session
(`sh -c 'echo $$; sleep 600'`), record its pid from the data plane, drop the `SessionMux`, then from a
**fresh** connection assert the process is gone (existed-before / gone-after, not a vacuous check).

### 12.28 One writer per connection

> **Every frame on a connection is emitted through the single per-connection writer (`send_framed` under one
> mutex on the guest; one `Arc<Mutex<SplitSink>>` on the host); no two threads write the transport
> concurrently.**

So multiplexed session frames from different pump threads never interleave-corrupt on the wire. Owner: guest
writer mutex + host writer sink. Gate: the concurrent-multiplexed-exec data-plane test — two sessions each
emit a large, self-identifying, window-filling stream on one connection, and the host reassembles **both**
intact with **zero** cross-attribution (an A line in B's stream, or a torn frame, reddens it).

### 12.29 A PTY session is a controlling-terminal session leader

> **A PTY session runs its command via `setsid` + `TIOCSCTTY` on the pty slave, so in-guest `isatty(0/1/2)`
> is true and a host `Winsize` delivers the new dimensions (`SIGWINCH`); output is the single merged master
> stream.**

The canonical `login_tty` sequence runs in `pre_exec` (async-signal-safe rustix syscalls only, one
SAFETY-doc'd `BorrowedFd::borrow_raw`); the master is `CLOEXEC` and the parent closes its slave so the master
EOFs (`EIO`) when the child exits. Owner: guest `run_session` (PTY arm). Gate: the KVM live PTY test — a
session running `test -t 0 && test -t 1 && stty size` reports a tty and the configured rows/cols on the data
plane, and a mid-session `resize` is reflected by a second `stty size` (a pipe session, as the
positive/negative control, reports "not a tty").

## 13. Hard-won lessons

Five meta-lessons recur across the implementation history (Appendix A) and are worth stating directly:
...
- **An "environmental" flake explanation is a hypothesis, not a diagnosis.** A sporadic "Agent … timed out"
  ...
  The discipline recurs whole: when the §22 suite showed 6 reds in the nested-virt + post-restore-reseed
  tests, the mechanism was named (a `kvm_intel` EPT-fault-degraded host) *and* the reds were **control-proven**
  by re-running them against a `git stash`ed baseline agent — identical failures — before attributing them to
  the host rather than the change (Appendix A).
- **The dev host *is* the KVM host — run the privileged suite, don't defer it as "forward work."** Across
  several layered components (v24–v26) the recurring mistake was framing the privileged suite as
  hard-to-run and hedging host-facing claims as "to be validated on a KVM host," when the capability runner
  is built and blessed, this host is KVM-capable, and one command runs it. The reflex was trained by the
  honest-hedge template ("KVM-host validation … forward work") reading as *some other* host. The fix
  (AGENTS.md rule 5, hardened): `scripts/review-preflight-priv.sh` printing `READY` means run the suites
  **now**; "forward work / not validated" is legitimate **only** when preflight prints `NOT READY`, and then
  you name the check that failed. Every one of §20/§21/§22's headline claims is backed by an actual
  `just test-privileged` / `just test-daemon` / `tests/{lineage,session}.rs` run recorded in §20.9/§21.8/§22.6,
  not a promise.
...
- **The privilege boundary is *who may execute the runner*.** `CAP_SYS_ADMIN` is root-equivalent in blast
  ...
  **developer-workstation** convenience, explicitly not for multi-tenant or production hosts (where the
  right answer is a privileged **setup broker**, §20). Two honest limitations: `CAP_SETPCAP` is *not* in the
...
**Privileged-window hardening gates (§20).** *Unit / pure (KVM-free, in `just ci`):* `vmm_seccomp_args`
golden output for every `(backend, VmmSeccomp)` pair + the two `Unsupported` combos (§12.21); `JailSpec`
defaults + clamps; `plan_broker_parent_drop` against its buggy inverses (drops effective/permitted/bounding/
ambient, sets no-new-privs, preserves the `kvm` gid — §12.23); the broker framed-protocol codec round-trip +
`> MAX_BROKER_FRAME_BYTES` reject; the broker dispatch call-order/residue-gone/sweep-only-dead tests against
the recording `Netlink`/`NftApplier`/`CgroupFs`/`OrphanScanner` fakes; the seccompiler deny-list compiles to
the expected syscall set; the `deny.toml` LGPL-`libseccomp`-family ban (a licensing hole tooling reports
green on, turned into a gate that can go red, §20.6). *Integration (KVM-free, root-free, in `just ci`):* the
jailer `/proc/self/status` stand-in gate (`NoNewPrivs:1`, `CapEff:0`, delete any prctl → reddens, §12.22);
the deny-list `mount→EPERM`/`write→ok` stand-in gate; the broker socketpair+fork round-trip over the real
transport. *Host-validated (KVM, `just test-privileged` / `just test-daemon`):* a seccomp'd + jailed live VMM
boots, execs, and tears down on each backend; the `vmcelld` broker cutover boots/execs/destroys through a
cap-dropped parent (12/12).

**OverlayStore-seam + lineage gates (§21).** *Unit / pure (KVM-free, root-free, in `just ci`):* `overlay` —
`ReflinkOverlayStore::clone_tree` produces a faithful, independent copy (write-clone ≠ mutate-master),
rejects a missing/non-dir `src` and a pre-existing `dst`, preserves symlinks, and `probe` agrees with an
actual clone's `CowSupport`; `RecordingOverlayStore` records `(src, dst)` and honors its configured
`CowSupport`. `lineage` — a root has gen 0 / no parent / empty ancestry; `branch` yields parent == the
branched node, gen + 1, extended ancestry; a 3-node chain has the right generations/parents/ancestries;
`is_ancestor_of` is antisymmetric and transitive; no node is its own ancestor; `LineageAllocator` hands out
distinct ids; an ineligible config (`branch`/`fork_from_vm` on a vhost-user config) is `Error::Unsupported`
at construction; `branch_creates_a_missing_target_dir` (the live-caught dir-create bug, §21.8). *Integration
against the recording fakes (KVM-free):* with a `RecordingVmm` + `RecordingOverlayStore`, `Lineage::fork`
materializes the clone through the seam into a **private** dir (§12.24), the clone gets a distinct vmid, and
the master is byte-unchanged; `Lineage::branch` returns a node with the correct lineage metadata; `fork_many`
inherits the §9.4 all-or-nothing teardown and concurrent-fan-out gate. *Host-validated (KVM,
`tests/lineage.rs`, CH+FC):* a live `fork` boots + execs with guest MAC == `mac_math(vmid)`; a `branch` of a
**diverged** running clone produces a node whose `fork` **sees** the marker while a `fork` from the root does
**not** — a data-plane positive/negative divergence control.

**Interactive-session gates (§22).** *Unit / pure (KVM-free, root-free, in `just ci`):* `vmcell-protocol`
round-trip of every new variant + a **discriminant-stability** test pinning the appended variants to indices
8..=15 (a reorder/removal reddens KVM-free, §4.1); guest `winsize_from(rows, cols)` field mapping (red on a
rows↔cols swap) and the shared `child_path(base)` PATH-augmentation used by both `handle_exec` and
`run_session` (one law; red if a session drops the `/vmcell-tools` prefix) and the open-failure →
`SessionStderr`+`SessionExit(127)` mapping; the AGENT-3 framing interop extended so a `SessionStdout{id,
payload}` framed by the guest decodes through the host's real `LengthDelimitedCodec` both directions
(over-cap reject); the host demux over an in-memory `tokio::io::duplex` pair (interleaved ids 1/2 + a stray
frame after id 1's `SessionExit` → each `Session` receives only its own frames in order and the post-exit
stray is dropped, §12.26). *Host-validated (KVM, `tests/session.rs`, all three backends, no `require_cap!`
skips — 14/14):* PTY `isatty`+initial-window+mid-session-resize with a pipe-session negative control
(§12.29); streaming-stdin round-trip through `cat`+EOF (§22.2); two window-filling self-identifying streams
multiplexed over one connection with zero cross-attribution (§12.28); a persistent `sleep` session's pid gone
after the `SessionMux` drops (§12.27).
...
- **Single-snapshot CoW for many clones — LANDED in v19 (§9.4); the seam + lineage LANDED in v25 (§21).**
  The zygote fan-out reflink-copy-on-write-copies the suspend dir per clone and restores each private copy
  (`Zygote`, `MicroVm::restore_cow`), so the in-place `config.json`/sidecar rewrites are per-clone and the
  master stays immutable (§12.12). Concurrent fan-out is gated on `restore_rotates_host_paths` (CH ✓; FC
  single-lineage, below). v25 lifted the per-clone CoW copy behind the injectable **`OverlayStore`** seam
  and added the **`Lineage`** fork/branch handle over `Zygote` (§21), both **KVM-validated**
  (`tests/lineage.rs` on CH+FC). **Residual:** (a) FC still has no per-clone host-path override, so a
  *concurrent* FC fan-out from one lineage is `Unsupported` (a single FC clone works); (b) **sparse-snapshot**
  (`SEEK_HOLE`) to shrink the on-disk suspend image is the un-taken pool-density lever (Appendix C) —
  orthogonal to CoW and still open; (c) a **non-reflink `OverlayStore`** (a shared content-addressed overlay
  pool), a daemon-exposed **fork/branch control-plane verb**, a lineage-aware orphan sweep, and CoW between a
  branch node and its parent image are the §21.8 increments the seam now admits.
...
- **Control-plane daemon — core verbs built; residuals catalogued in §18.9.** The productization seam
  ...
  warm-pool/zygote manager, a UDS transport (TCP loopback ships now), JWT / per-key scopes (a single
  all-scopes key ships now), `pause`/`resume` routes, writable-scratch-from-artifact disks (daemon extra
  disks are read-only), artifact GC/quota, and CLI-stub removal — are catalogued in §18.9. (The **setup
  broker** is no longer a residual: it is built, KVM-validated, and the **default** for `vmcelld` — §20.5,
  below.)
- **Privileged-window hardening — built (§20).** Layers 1–2 (every VMM under its own seccomp via
  `vmm_seccomp_args`; the jailer-equivalent `JailSpec`/`apply_jail`) are built and **KVM-validated** on all
  three backends (`just test-privileged`). Layer 3 (the setup broker) is built, **KVM-validated**
  (`just test-daemon` 12/12), and the **default** for `vmcelld` (the cap-dropped parent serves HTTP,
  §12.23). Remaining: the *thin* broker (shrink the privileged code surface), the seccompiler deny-list and
  `clear_ambient_caps` defaults (blocked on the fd-passing/uid-drop increment, an empirical KVM finding,
  §20.9), and the jailer's chroot/uid-drop increment.
- **Interactive sessions — built at the library + CLI (§22).** PTY / streaming-stdin / multiplexed-exec
  sessions over the vsock control plane ship and are **KVM-validated** on all three backends
  (`tests/session.rs`, 14/14) with a minimal `vmcell-cli run --stdin`/`--tty` surface. Remaining (§22.7):
  daemon streaming sessions (a WebSocket/chunked transport + a `SessionId` sub-protocol, plus streaming
  `VmEngine` ops), a raw-mode interactive CLI, per-session output backpressure (the host per-session queue
  is unbounded — a recorded host-trusted trade, §22.4), and richer PTY `StdinEof`.
...
the agentic and serverless domains share — **shipped in v19 as the zygote fan-out** (§9.4) and grew its
**injectable `OverlayStore` seam + `Lineage` `fork()`/`branch()` handle in v25** (§21): a `Lineage` node is
an immutable single-snapshot zygote, `fork` mints a live child, and `branch` freezes a *running* descendant
into a new node — an acyclic tree of provenance. What remains *future* on top of it: a **`fork()`-style
divergence API** that clones a *running* VM at an **arbitrary** instruction point (not just an agent-ready or
branch snapshot), per-clone **overlay divergence tracking**, and a **non-reflink `OverlayStore`** (a shared
content-addressed pool) the seam now admits (§21.8). The
...
hand-out policy, gated on the §9.4 fan-out capability); a generic **vsock↔TCP port-forward bridge** (the
in-guest model-proxy transport); **observability + resource controls** over OTLP;
...
Two items previously on this list have **graduated to built** and left it: **privileged-window hardening**
(each VMM's own seccomp, a jailer-equivalent, and the setup broker — now **§20**, the broker the default for
`vmcelld`), and **persistent interactive PTY sessions** (now **§22**, the PTY / streaming-stdin /
multiplexed-exec session layer, shipping at the library + CLI).
...
```
  vmcell-privilege ◀── vmcell-test-runner        (lean tier; no vmcell edge)
        ▲
        └────────────── vmcell-daemon ──▶ vmcell (0.9.0, host stack; cloud-hypervisor+metrics+pipeline)
                             ▲
...
**Where the daemon runs its privileged work — behind the setup broker by default (§20.5).** The netns/tap/nft
bring-up (§6.4) still runs through `MicroVm::start`/`restore` using the caps the cap-holder holds, but which
process is the cap-holder changed in v24: `vmcelld` now **forks the setup broker by default**. The broker
child keeps the three caps and owns the VM `Registry`; the parent **drops all three caps**
(`apply_broker_parent_drop`), then binds and serves the HTTP surface with **no capabilities**, forwarding
every VM op to the broker over the multiplexed `VmEngine` JSON RPC (§20.5). A memory-safety or logic bug in
the request parser can therefore no longer reach the caps (§12.23) — the hardening §17 named as "forward
work" is **built and KVM-validated** (`just test-daemon` 12/12 with the parent holding no usable caps). The
retain-caps single-process form described above is now the `--no-setup-broker` **fallback** (§12.14), not the
default. The remaining *thin*-broker refinement (shrink the privileged code surface itself, rather than only
moving the caps off the network surface) is §20.9.
...
plane**, never a silent skip. **Validated on the KVM host** — 11/11 green in the initial single-process run,
and **12/12 after the §20.5 setup-broker cutover** (the whole HTTP surface now runs cap-dropped, forwarding to
the broker, §20.9) (+ the `vmcell` unit suite via
...
- **`pause`/`resume` are not in the v1 surface.** The registry + handle support them (the seam has
  ...
  small follow-up.
- **The *thin* setup broker (shrink the privileged code surface).** The setup broker itself is **built,
  KVM-validated (`just test-daemon` 12/12), and the default** (§20.5, §12.23): `vmcelld` forks it, the broker
  child holds the three caps and owns the VM `Registry`, and the HTTP-serving parent **drops all caps** and
  forwards VM ops over the `VmEngine` RPC — so the network-facing surface holds *no* capabilities. What
  remains forward work is the *thin* variant that moves VMM-driving to the parent behind `SpawnVmm`+pidfd (a
  cross-process `MicroVm` refactor), shrinking the privileged code from "owns the whole registry" to "the
  setns/spawn primitive" (§20.9). `--no-setup-broker` selects the retain-caps single-process fallback
  (§12.14), which additionally leans on bind-loopback/UDS-by-default, auth-by-default, the key-file perms
  check, and each VMM's own native seccomp (§20.3).
...
- **The daemon co-locates the caps with the network-facing HTTP surface (before this component).** In the
  single-process form (`--no-setup-broker`, §12.14) `vmcelld` holds all three caps *and* terminates the REST
  API in one process (§18.2), so any memory-safety or logic bug reachable through the request parser executes
  with the union of the three caps in the initial user namespace. Layer 3 below is what removes that
  co-location; §18.2 now describes the broker as **built and the default**, with this section its
  specification.
...
Each layer is independently useful and independently gated; none depends on the next shipping. All three
ship: Layers 1–2 are KVM-validated on every backend, and Layer 3 (the broker) is the default for `vmcelld`,
KVM-validated at `just test-daemon` 12/12 (§20.9).
...
The invariants this component adds — **§12.21** (every backend under its own seccomp; one predicate decides
the flag), **§12.22** (one jail, applied by every VMM spawn; the pure spec is separate from the syscall
edge), and **§12.23** (in the broker model the network surface never holds the caps, and the cap-holder
never parses network input) — are stated in full in §12, the canonical home for cross-cutting laws.

### 20.8 Quality gates (added to §14)

The gates for this component are folded into §14 (the *privileged-window hardening gates* subsection): the
`vmm_seccomp_args` golden-args + `Unsupported`-combo tests, the `JailSpec` defaults/clamps, the
`plan_broker_parent_drop` inverse tests, the broker framed-codec + dispatch-against-fakes tests, the
seccompiler deny-list + the `deny.toml` LGPL-`libseccomp` ban, the root-free `/proc/self/status` and
deny-list `mount→EPERM` jailer stand-in gates, the broker socketpair+fork transport round-trip, and the
KVM-host live-boot / broker-cutover validation. See §14.
...
The invariants this component adds — **§12.24** (every CoW clone is materialized through the injected
`OverlayStore`, into a private copy, never the master) and **§12.25** (a lineage is immutable and acyclic;
a branch’s ancestry is its parent’s plus its parent, generation strictly increases, no node is its own
ancestor) — are stated in full in §12, where §12.12’s master-immutability law is also extended to branch
nodes.

### 21.7 Quality gates (added to §14)

The gates for this component are folded into §14 (the *OverlayStore-seam + lineage gates* subsection): the
`overlay` faithful/independent-copy + probe tests and `RecordingOverlayStore`; the `lineage` ancestry /
`is_ancestor_of` / distinct-allocator / eligibility / `branch_creates_a_missing_target_dir` unit tests; the
recording-fake integration tests (fork through the seam into a private dir, branch metadata, `fork_many`
all-or-nothing); and the KVM-host `tests/lineage.rs` positive/negative divergence control. See §14.
...
The invariants this component adds — **§12.26** (session I/O is channelized and terminally-framed),
**§12.27** (a connection owns its sessions), **§12.28** (one writer per connection), and **§12.29** (a PTY
session is a controlling-terminal session leader) — are stated in full in §12.

### 22.6 Quality gates (added to §14)

The gates for this component are folded into §14 (the *interactive-session gates* subsection): the
`vmcell-protocol` round-trip + discriminant-stability tests over all eight appended variants; the guest
`winsize_from` / shared `child_path` / open-failure-mapping / `SessionStdout` framing-interop tests; the
host demux interleave-and-drop-post-exit test; and the KVM-host `tests/session.rs` (PTY+resize, streaming
stdin, multiplexed exec, connection-drop residue — 14/14 on all three backends). See §14.
...
**Reversals in the layered components (v20–v26).** The daemon (§18), device surfaces (§19), and the three
hardening/seam/session components (§20–§22) added no architectural change, but each surfaced a load-bearing
reversal a live run caught that static review would have missed — the same discipline as reversals 1–8:

- **Reversal 9 — the jailer's `clear_ambient_caps` (§20.4/§20.9).** *Prior belief:* the VMM "already runs
  with no capabilities," so the jailer should clear its ambient set. *Finding:* on the `vmcell-test-runner`
  path the three caps live in the **ambient** set, so a fork+exec'd VMM **inherits** them — and a restored
  Cloud Hypervisor's `TapSetMac` (`SIOCSIFHWADDR`) and Firecracker's tap re-open both `EPERM` without
  `CAP_NET_ADMIN`; clearing ambient reddened every restore-with-tap test (cold boot survived because it does
  not re-set the tap MAC that way). *Where it landed:* `clear_ambient_caps` defaults **off** (an opt-in
  reserved for the future fd-passing/uid-drop path); `no_new_privs`/`RLIMIT_CORE=0`/`non_dumpable` stay on,
  validated non-breaking across all three backends — "defaults get the strictest scrutiny" + "validate on a
  KVM host" catching a real regression.
- **Reversal 10 — the broker RPC is JSON, not postcard (§20.5).** *Prior belief:* the broker bridge should
  use `postcard`, the `vmcell-protocol` framing discipline. *Finding:* `create` hung while `get`/`list`
  passed — the reused daemon DTOs carry `#[serde(skip_serializing_if)]`/`default` fields that round-trip only
  in a **self-describing** format, so postcard (non-self-describing) byte-misaligned the reply frame and the
  request never resolved. *Where it landed:* the `VmEngine` RPC speaks JSON (the format the HTTP API already
  speaks), with a KVM-free `bridge::tests` round-trip over every op incl. `create` so a format regression
  reddens.
- **Reversal 11 — `Lineage::fork_from_vm`/`branch` must create the target dir (§21.8).** *Prior belief:*
  mirror `Zygote::suspend`'s "caller creates the dir" contract. *Finding:* a `branch` into a not-yet-created
  dir fails loud in the backend (CH "Destination is not a directory", FC "Cannot perform open on the snapshot
  backing file: No such file or directory") — invisible to the `FakeVmm` unit gate (its snapshot is a no-op
  that never opens the dir), caught **only** by the live VM suite. *Where it landed:* both verbs
  `create_dir_all` the destination first, guarded by a new KVM-free `branch_creates_a_missing_target_dir`
  unit gate — the canonical "green static review proves little" case.

These join the standing rule that a §22 host-environmental cluster (the `kvm_intel` EPT-fault-degraded
nested-virt + virtio-rng reseed tests) was **control-proven** not-the-change by re-running the 6 reds against
a `git stash`ed baseline agent (identical failures, same host kernel mechanism) — §13's "environmental is a
hypothesis, not a diagnosis" applied, not assumed.
