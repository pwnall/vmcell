...
> delta register** — nine deltas, replacing the landed v28 register — is the exact boundary between "as
> built and validated at v29/0.12" and "directed by this revision"; deltas 1–8 land as one breaking pass
> (`vmcell` 0.12 → 0.13), delta 9 (FR-V5) is separable and explicitly last. New sections: **§5.6** (the
...
out the bounded per-byte read (`Timeouts::connect_ok_read`) into a typed timeout naming the port. EOF
propagates in both directions (half-close forwards). Two documented caveats: a *user* listener gets no
...
// vmcell oci2erofs … --inject dest=/usr/local/bin/acme-daemon,src=./acme,mode=0755   (repeatable)
```
...
`GuestToolsStage` builds the helper and the packer injects it with `ip`/`curl`/`kvm-ok`/`echo-server`
symlinks (the last is v30's test listener for the dial and segment gates — `--vsock <port>` /
`--tcp <addr:port>`, a real echo server, §3.2/§6.5); the agent prepends its dir to the exec `PATH`. The rootfs cache key folds the helper's content, so a helper
...
(`vmcell::artifact::build_labelled_kernel(label, &env)` — a thin assembler of
`ResolvePinsStage → KernelStage`/`InVmKernelStage` that any git-dep workspace can call; it deliberately
...
workspace tree, §10.2). The default kernel is byte-unchanged for consumers that do not opt in. Two
fail-louds the current tree lacks: requesting a label or fragments with `--kernel-source prebuilt` is a
typed error (today `kernel_stage()` silently drops both and hands back the default seed), and a
labelled build reports which producer it used.
...
**failure classification**. A kernel missing a baseline symbol today fails `boot.kernel_banner` or
`boot.agent_ready` by timeout with a raw serial tail; v30 adds a pure serial-log classifier that maps
the known contract-violation signatures onto the §5.4 clause they break — `VFS: Unable to mount root
fs` → the erofs/root symbol set, a vsock `EAFNOSUPPORT` → `CONFIG_VSOCKETS`, no `Linux version` banner
at all → not a bootable PVH `vmlinux` — so the failed check *names the missing contract clause* instead
of handing back a timeout (the first concrete consumer of the §17 "structured serial fault classifier"
idea, scoped to exactly these signatures; the classifier is pure and red-on-inverse-tested on canned
logs, and `validate()` against a garbage kernel file stays the cheap live red path the smoke test
already exercises). A boot failure the classifier does *not* recognize still reports the named check,
the deadline that expired, and the serial tail with a pointer to the §5.4 contract checklist — the
residual class fails named-and-loud too, and a newly-understood signature grows the classifier, never
the timeout path. The battery stays bounded — every check carries an `Instant` deadline
(outer-bounds-inner, §9.4) — so "fails loudly, not by hanging" holds by construction.
...
which every backend must acknowledge to compile, and the tap-vs-NAT device wiring routes through one
shared `net_uses_tap(cfg)` predicate covering `Privileged | Segment` so no backend re-derives the mode
from an ad-hoc match (§6.5).
...
pre-exec path the per-VM netns uses — a different netns *name*, zero new spawn logic; each backend's
tap-vs-NAT device wiring routes through the one shared `net_uses_tap(cfg)` predicate (§6.2), so
`Privileged` and `Segment` take the identical tap arm. Isolation between
...
`segment_ip_math(seg_octet, slot)`, a unit-tested sibling of `ip_math` in the same module. The guest
...
compile-time channel that forces every backend to acknowledge segments (§6.2). Guest MACs stay
`mac_math(vmid)` — vmids are host-unique, so member MACs are bridge-unique by the existing
collision-freedom law, and the scheme stays outside the NAT's reserved-MAC image.
...
the privileged suite's liveness-blind test-start sweeper already removes every `<prefix>-`-prefixed
netns, segments included.
...
in `--tcp` mode is the listener — delta 7 ships it, so land 7 first; guest-tools `curl`/the dialer side
drives it), with a third off-segment VM's dial to **both** members refused/timed out as the negative
...
`oci2erofs IMAGE@DIGEST [--inject dest=…,src=…,mode=…]` (§4.2), the live-handle lifecycle verbs
...
`vmcell oci2erofs … --inject …`) so the CLI half of the contract, which `semver-checks` cannot see,
...
`vmcell build` / `oci2erofs --inject` and point your harness at it via `VMCELL_ROOTFS` +
...
capabilities, §2.5, stay auditable — 5 of them recorded as `require_cap!` skips on its opt-in
`just test-crosvm` matrix run). A per-flag capability-honesty test pins every `VmmCapabilities` field —
all nine once delta 9 lands (`snapshot_restore`, `lazy_restore`, `virtio_fs_shares`,
...
touching the NAT's five-invariant datapath or its open bring-up flake); and daemon/REST exposure of
...
natural extension when a consumer needs it. `harness::ch_bin()` duplicates
`vmcell::artifact::ch_binary_path()` (same env var, same default) — a one-law consolidation target.
USB-passthrough guest-side coverage beyond enumeration + one class smoke (per designated device class)
...
---
## 18. Delta register: changes from the validated v29 build (the downstream-platform pass)

The body of this document describes the **target** design. This section is the explicit list of where
that target differs from the last validated implementation (v29, `vmcell` 0.12), so a maintainer holding
the running code knows exactly what to change and why. **These nine items are specified here but not yet
built**; the implementer executes them, then reconciles the result in `docs/implementation-notes.md`.
(The previous register — the eleven-item v28 pass — is fully landed; its as-built record is
implementation-notes "v28 — the 0.9 → 0.10 delta register, as built". This register replaces it.)

**Bundling and order.** Deltas 1–8 are **one breaking release, `vmcell` 0.12 → 0.13** (the breaking
edges: `pack_erofs_with_injection` gains a parameter, `PerVmResources` gains the exhaustive-by-design
...
- **Premises are verified anchors, not memory.** (The v28 record: delta 5's "no consumer" and delta 6's
  "none is known to use it" were both empirically false.) Every current-state premise below carries its
  symbol anchor, verified against the 0.12 tree on 2026-08-11; if the tree has moved when you implement,
  re-verify the anchor before cutting, and treat a stale premise as a stop-and-check, not a nuisance.
...
3. **The labelled-kernel build path, completed** *(FR-V1; §5.5–§5.6, §10.1)*. *Premise:* fragments are
   ...
   **empty bytes** in `cache_key()` (the `unwrap_or_default` at `kernel.rs:178-190`) while `run()`
   ...
   git-dep callers. *Why:* this is FR-V1's build half — the P-blocking item — built almost entirely
   from shipped parts; the resolved-config sidecar is the anti-silent-drop half of "assert against the
...
4. **The validation battery names what a bad kernel is missing** *(FR-V1; §5.6)*. *Premise:*
   ...
   classifier mapping the known §5.4 contract-violation signatures (`VFS: Unable to mount root fs` →
   root-fs symbols; vsock `EAFNOSUPPORT` → `CONFIG_VSOCKETS`; no `Linux version` banner → not a
   direct-boot `vmlinux`) into the failing check's message, plus `KconfigValues::parse` for
   resolved-config assertions; the battery is promoted to contract surface (delta 2). *Why:* "fails a
