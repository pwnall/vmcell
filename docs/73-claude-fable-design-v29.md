...
> changed, and there is no new §18 delta.
...
| cold boot (p50, §16) | ≈316 ms | ≈764 ms | ≈965 ms | — *(unmeasured)* |
...
most-proven nester; crosvm (v29) is a fourth boot-first backend whose tier is undetermined until measured.
...
- **`vmcell-bench`** — the cross-backend `bench-vm` harness, the one composition root wiring all four
  backends (each secondary backend behind an optional `firecracker`/`qemu`/`crosvm` feature; `crosvm` is
  out of the default set until validated). Kept outside `vmcell` so the dependency graph stays acyclic.
...
`just test-crosvm` matrix run). A per-flag capability-honesty test pins all eight capability flags
(`snapshot_restore`, `nested_virt`, `virtio_fs_shares`, `virtio_console`, `disk_io_throttle`, the two
seccomp-log unsupporteds, `restore_rotates_host_paths`) to the backend that actually supports them, so a
flag that lies (advertising a capability the backend lacks, or vice versa) goes red. **Zero selected tests
is a CI failure**
