...
`docs/83-claude-fable-design-v33.md`, and **that reissue landed in the same commit**
(`git show c34f9c2 -- docs/83-claude-fable-design-v33.md` carries every one of those sentences as an
...
| C3 | fixed | `build_labelled_rootfs` / `build_labelled_handler` do not exist and are not going to; the `RootfsStage::labelled` / `GuestToolsStage::labelled` constructors are the decision, recorded in `docs/implementation-notes.md`. **The design lines landed with the pass:** §10.4's contract list names the two constructors plus the `vmcell build --rootfs-label`/`--handler-label` verbs that drive them (`docs/83-claude-fable-design-v33.md:3425-3428`), and §10.5's "Where selection lives" names the same shape *and* records the second shift — why no thin assembler mirrors `build_labelled_kernel` (`:3602-3613`). |
