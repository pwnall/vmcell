# vmcell — Design Document (v32)

> **v32 (2026-08-15) is the post-`docs/81` re-base.** The Opus code review of the 0.13 tree
> (`docs/81`) landed in three fix waves, and its §7 recorded what the *document* got wrong rather
> than the code. AGENTS.md's rule — **no standing errata; a correction is folded into the body when
> the design is reissued** — is why this revision exists. v32 changes **no architecture**, adds no
> section, and renumbers nothing: it re-states the body as the system is, at the places the two
> disagreed. Every correction below was re-verified against the tree while writing it, not taken
> from the review's line numbers (three waves of edits had moved them):
>
> - **The CI runner.** §5.6, §15.4 and §18 delta 5 said the integration job runs on a
>   `[self-hosted, linux, kvm]` runner. It does not, and never did: no such runner was ever
>   registered, so the job sat queued until it was moved onto GitHub-hosted runners. `ci.yml`'s
>   `test-integration` is `runs-on: ubuntu-24.04`, the label triple appears in no `runs-on:` in the
>   file, and the downstream example's live KVM leg is a **step inside that hosted job**, not a
>   separate self-hosted one.
> - **§9.2's module map** now matches `crates/vmcell/src/`: `vmm/` carries only the primary backend
>   (Firecracker and QEMU were extracted to their own crates back at §9.1), `net_sys.rs` holds
>   **two** unsafe edges — `setns_net` joined `TUNSETPERSIST` — and `vmm/usb.rs` is listed.
> - **§9.3's `VmConfig` roster** gains `vsock_transport`, which §17 and §2.4 already described as
>   shipped.
> - **§11.3's delete** is `delete_artifact_if_unused` — the in-use check and the unlink under **one**
>   hold of the VM-table lock — not the check-then-delete two-step, which reopened the TOCTOU.
> - **§11.5 / §13 P5** describe the parity gate that now exists instead of the one that was claimed:
>   the router is a **fold over `API_ROUTES`** (so the mounted surface is the documented table by
>   construction, not by assertion), and `components.schemas` really carries an `ErrorBody` whose
>   `error` enum is generated from `ErrorKind::ALL`, so "every named schema exists" has something to
>   resolve.
> - **§3.2's `SessionMux` writer** is the shipped one: an `mpsc::UnboundedSender<Bytes>` feeding a
>   sole-owner writer task, not the `Arc<Mutex<SplitSink>>` the sketch showed. Both satisfy law C4;
>   the channel form holds no lock across an await.
> - **§11.2 and §15.5** state the runner's privilege transition in its shipped order — **uid drop
>   before the ambient raise** — which is the security-critical direction the code deliberately
>   takes and its own tests assert. The document had it backwards in both places — and contradicted
>   itself, since §15.5 went on to name the correct order a few lines later. §15.5 now states the
>   whole transition as the ordered step list the code emits.
> - **§11.4's start-up sweep** loses an over-claim: the sweep is cross-process liveness-**blind**, so
>   "the empty set can never sweep a resource in use" holds because the daemon owns its prefix, not
>   because nothing is live.
> - **§17 is re-checked, not copied forward.** The integration harness's `computed_cgroup_name` is
>   **closed** (it routes through the one naming law). And the NAT bring-up flake's recorded
>   *mechanism* is **withdrawn**: `Listener::new` **is** the bind, called on the caller's thread
>   before the worker thread is spawned, so the named fix would retire nothing. The ~10% flake is
>   real; its mechanism is open, and §17 now says so rather than carrying a diagnosis the tree
>   contradicts.
> - **§10.4's downstream `setcap` route and §15.5's account of the gate that reads it** carry the
>   four-capability `BLESSED_FILE_CAPS` (the three delivered caps plus the transient `CAP_SETPCAP`),
>   corrected during the `docs/81` waves and reissued here.
> - **§9.1 and §18 stop restating the crate version.** The `docs/81` waves cut a breaking release, so
>   the embedded figure went stale for the second time in two revisions; the body now points at
>   `crates/vmcell/Cargo.toml` and its comment changelog, which is where the number is *produced*.
>
...
> broke on this very reissue — and both **error** rather than pass vacuously when they find nothing,
> so a reissue moves the design without breaking either, while a *deletion* of what they read
> reddens. Superseded revisions live in
> `docs/historical/` (v31 is `docs/historical/79-claude-fable-design-v31.md`), frozen at the bytes
> they shipped with; neither gate descends there.
>
> **Where this document came from.** **v31 (2026-08-14) was the post-landing re-base** of the v30
> pass: the register's nine deltas built, live-validated, and reconciled (`vmcell` **0.13**), the
> `docs/78` review landed in two fix waves, and the body re-stated against the shipped system —
> §3.2's measured four-backend half-close table, §6.5's corrected bridge-MAC and segment-sweep
> premises, the two-channel tap wiring in §6.2/§6.5, §6.2's sixth NAT invariant (the guest→host
> drain consumes only the contiguous span), §8.1's one config-only eligibility predicate with its
> falsified `restore()`-rejects premise named as such, §2.3/§8.2's Firecracker tap rebind and the
> scoping of `restore_rotates_host_paths` to the vsock/serial paths, §12.3's three folded deny-list
> carry-overs, and §18 as the **landed** record of the v30 pass with its register conventions kept
> as standing rules. v32 carries all of it forward. Before that: v28 restructured v27 for a developer learning the system in order to
...
connectable within 2 s. This is the fifth shipped-fact premise this project has recorded and then
falsified; the §18 convention (verify a premise's anchors before cutting) is why it was caught here
...
**All nine items are landed and reconciled** — the pass shipped as `vmcell` 0.13, and the tree has moved
past it since (the crate's `Cargo.toml` ledger is the current fact, never this line) — and the per-delta as-built record —
...
5. **The out-of-tree example workspace** *(FR-V1/FR-V6; §5.6, §10.4)*. *Premise:* no downstream-shaped
   ...
   **harness getters' downstream contract** (§10.4 — `get_vmlinux`/`get_rootfs` return the named paths
   ...
   the exact documented CLI invocations (§10.4) — on every push, in its own KVM-free job; the KVM leg
   is a step of `ci.yml`'s `test-integration` job, on a GitHub-hosted runner like every other job in
   the file (§15.4 — the `[self-hosted, linux, kvm]` selector this item originally named appears in no
   `runs-on:` and never ran). *Why:* the serial-nexus lesson the FR cites verbatim — an
