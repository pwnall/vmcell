...
- **Full apt chain — `mmdebstrap` inside a builder micro-VM.** Build a builder rootfs via the OCI source,
  ...
  against the builder base image's own `debian-archive-keyring` — an equivalent trust root pinned
  transitively by the base-image digest, not a separately-pinned keyring file (M-ART-5) — and
  `mmdebstrap`, `apt`, `gpg`, and the shell all leave the host entirely. This source is **library-present
  but deferred**: the CLI `build` verb wires only the OCI source today, and the in-VM path runs only inside
  a fully-booted builder micro-VM that is not yet wired end-to-end (§16).
...
- **`vmcell`** — the library (plus the `vmcell` CLI and the `bench-vm` harness), one package carrying the
  host feature stack (§10.5). The workspace crates version **independently**, not in lockstep — `vmcell` is
  at 0.4.0 and `vmcell-protocol` at 0.3.0 (the additive `Resync`/`ResyncAck` variants and the
  `AgentClient` `Timeouts`-param arity change drove those bumps); the three binary members version lower.
...
- **In-VM `mmdebstrap` source is library-present but deferred.** `RootfsBuildSource::Mmdebstrap` exists, but
  the CLI `build` verb wires only the OCI source — the mmdebstrap path only runs inside a fully-booted
  builder micro-VM, which is not yet wired end-to-end. Its apt verification uses the pinned base image's own
  `debian-archive-keyring` (an equivalent trust root pinned transitively by the base-image digest);
  passing an explicitly pinned keyring is forward work for when the path is un-deferred.
