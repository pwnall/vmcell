...
The footprint and on-disk-size benchmarks added to the plan — guest RAM footprint and the musl-vs-glibc agent comparison (§13.3), and rootfs image size and suspend-state size (§13.6) — are **defined but not yet run**; their results land in this subsection once measured on the pinned substrate.
