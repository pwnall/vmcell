...
pub enum NetConfig {
    ...
    /// interception happens at L4 inside the NAT.
...
- **Newly-adopted crates still face the gate.** The graduated experiments (§10) add `am-fs-erofs` (erofs build) and `smoltcp` + `vhost-user-backend` (rootless NAT). `smoltcp` (0BSD) and `vhost-user-backend` (rust-vmm, Apache-2.0) are well-established; **`am-fs-erofs` is obscure — confirm its license and maintenance via `cargo-deny` before it stays in the default path**, keeping `mkfs.erofs` as the fallback (the `rustables` incident is the cautionary precedent).
...
pipeline = [
    ...
    "dep:tar", "dep:am-fs-erofs", "dep:flate2", "dep:lzma-rs", "dep:zstd",
...
2. **Root filesystem** (per profile): `mmdebstrap` minbase + the test toolchain + `imp-guest-agent` (injected via `--customize-hook=copy-in`), packed as a **single read-only erofs image**, built by streaming the `mmdebstrap` tar output directly into the `am-fs-erofs` crate in memory (§10 Exp 3) — which also avoids creating device nodes or root-owned files on the host, so the build runs unprivileged; `mkfs.erofs` is the fallback. That one artifact serves *every* path — cold boot, concurrent shared mounts, and the snapshot tier — because erofs over virtio-blk is read-only, shareable, and snapshot-eligible. (v1 specified a dual emission, erofs *and* a separate block image; with erofs-over-virtio-blk the two collapse into one.) **Imp's own binaries are *not* baked in** — they arrive over the `imp-bin` virtio-fs share, so a new Imp build does not invalidate the rootfs. **Build-host note (observed):** `mmdebstrap` invokes a shell and fails under Ubuntu's default `dash`; run it with `SHELL=/bin/bash` (and ensure `/bin/sh` → `bash`). The `imp-testing build` command preflight-checks this and halts with a clear message if it is misconfigured. (This host quirk is also part of the case for the OCI-rootfs experiment in §10.)
...
- **Stages 1..n — deterministic given inputs.** Each stage's output is fully determined by its inputs + the pins. Examples: *fetch+verify kernel source*, *configure+compile `vmlinux`*, *`mmdebstrap` rootfs at the pinned snapshot*, *copy-in `imp-guest-agent` + CA*, *erofs pack (stream the rootfs tar into `am-fs-erofs` in memory; `mkfs.erofs` fallback)*, *boot+snapshot*.
...
| 4 | `mmdebstrap` → OCI image | **Postponed** | Would drop apt signing-chain verification; deferred. |
...
**Experiment 4 — OCI-image rootfs (`oci-distribution` + `oci-spec`). Status: postponed.** *Goal:* pull a pinned Debian image by digest instead of assembling with `mmdebstrap`, eliminating the `dash`/`SHELL` quirk. **The critical distinction is OCI-as-build-source vs OCI-as-runtime-mechanism — opposite performance profiles.** *As a build-time source* (what this experiment means): `oci-distribution` does registry HTTP pulls in the pipeline — **no Docker/containerd daemon** — and the extracted layer tar feeds the *same* `am-fs-erofs` packer to produce the *same* erofs image booted today. The guest never sees OCI; direct-kernel boot, snapshot/restore, and shared-RO-erofs page-cache density are all unchanged, so this is **performance-neutral on the hot path** (per-test running time and VM density), and build time may even drop by skipping mmdebstrap's per-package dpkg unpack/configure. The usual worries don't bite here: a fatter `*-slim` base costs host disk and a one-time cache warm-up, not per-test time, because erofs over virtio-blk is demand-paged and shared (boot touches only the working set, once, for all VMs); the agent bypasses distro init (`init=/sbin/imp-guest-agent`) so a larger userland doesn't grow the boot working set; snapshots capture guest *RAM*, not the rootfs disk; and Imp's binaries arrive over the `imp-bin` virtio-fs share rather than the image. *As a runtime mechanism* (containerd + snapshotter + runc + overlay-of-layers) every worry is real — extra daemons, per-pull layer decompress/assembly, an overlayfs-of-many-layers with worse inode/page-sharing — and it would break the single shared erofs and snapshot/restore the performance story rests on; **this is not what the experiment means and must not be pursued.** *Reason deferred:* once that confusion is cleared, the real trade is **not performance vs signing — it is apt-chain signature verification vs build-time convenience.** OCI-as-source removes the pipeline's `InRelease`/`Release.gpg` + rPGP verification *stage* (§7) in exchange for registry-digest trust, and weakens reproducibility (a digest depends on registry retention, whereas snapshot.debian.org + `mmdebstrap` rebuilds from a timestamp from first principles). Since the upside lives entirely in the offline pipeline (weighted lightly by the requirements) and the cost is a genuine supply-chain reduction, `mmdebstrap` stays the default. *If revisited:* adopt OCI **only as a build-time source feeding the same erofs packer**, and book the signing-chain drop as the explicit thing being paid for. (Orthogonal and separately useful: pushing the *built* erofs image to a registry as an OCI artifact is OCI-for-distribution — handy for sharing pre-built rootfs across CI runners, independent of both the rootfs source and provenance.)
...
- `lifecycle.rs` (M1/M8): **ordered `Drop` teardown on `panic`** leaves zero residue — no CH process, netns, cgroup, or socket survives, asserted via the §5.3 sweeper/registry — review §6.2 / §4.8.
...
- **Performance/density** (§3.4/§9): `criterion` benchmarks for cold-boot, restore, idle RSS, and the concurrent ceiling — tracked metrics, not gates, consistent with §3.4's "quote no number as fact."
