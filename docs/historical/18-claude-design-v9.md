...
- **Egress-proxy overhead, privileged vs rootless** — added latency/throughput of **tap + TPROXY** (privileged) against the **in-process smoltcp L4 NAT** (rootless, §10 Exp 5). §9 already flags the smoltcp datapath as lower-*fidelity*; this puts a *cost* number next to the fidelity/convenience trade so it is decided on data, not vibes.
