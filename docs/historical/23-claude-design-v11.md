...
pub trait Vmm: Send + Sync {
    ...
    /// rejects a boot of a restored VM with 500 "VM is already created"; Firecracker (validated
    /// v11, MMIO mode) starts a fresh process and POSTs `/snapshot/load` with `resume_vm:true` —
    /// pause/resume is `PATCH /vm`, NOT `PUT` (PUT yields a cryptic 400), and you may NOT (re)configure
    /// drives/vsock before or after load, so per-restore identity uses *relative* snapshot paths
    /// resolved against each sandbox's working dir (the multi-restore-from-one-snapshot trick); QEMU
    /// uses incoming migration / `loadvm`, but only where its vsock is NOT the external vhost-user
    /// `vhost-device-vsock` daemon — that daemon can't migrate, so QEMU's rootless+vsock config reports
    /// `snapshot_restore:false` (§15). All restorable backends reset the vsock device on restore — see
...
- **`net`** — Two implementations behind `NetConfig` (see §6.3/§6.4 and §9):
  - `tap` (**privileged**): a per-VM network namespace, a `veth`/tap pair, and a `/30` (`10.200.<vmid>.0/30`, host `.1`, guest `.2`) via `rtnetlink`; a transparent redirect of guest tcp/80,443 (and optionally udp/443) to the host proxy, plus `drop`/`log` rules. The ruleset is **rendered in Rust but applied via the external `nft -f -` binary** — no permissive pure-Rust nftables crate covers the TPROXY/`socket` expressions (`rustables` is GPLv3; see §5.4 and the §10 experiment). **(observed, v10 — deviation) The third pass applied an `iptables REDIRECT` (`-j REDIRECT --to-ports`) instead of nft `TPROXY`, "for simplicity / fewer kernel deps."** This is a legitimate simplification, and the design's earlier blanket claim that "REDIRECT cannot preserve the original destination" was **overstated**: REDIRECT *does* recover the original IPv4 TCP destination via `getsockopt(SO_ORIGINAL_DST)` (and an HTTP/HTTPS proxy can also read it from the `Host` header / `CONNECT` target). TPROXY's genuine advantages over REDIRECT are narrower but real — it carries the original destination *in the socket* (no conntrack lookup, which is the cleaner story when the proxy listens outside the guest's netns), it preserves the original *source*, and it handles **UDP** (so transparent QUIC/HTTP-3 on udp/443; REDIRECT is TCP-only, forcing udp/443 to be blocked into TCP fallback). **Resolution:** accept REDIRECT for the current HTTP/HTTPS-over-TCP egress scope; keep TPROXY as the documented upgrade path the moment UDP egress is needed or `SO_ORIGINAL_DST` proves fragile across the netns boundary. The assertion that matters (and what the test should check) is that *the proxy observes the guest's intended destination* — satisfied by either mechanism — not which mechanism is used.
...
3. **Warm snapshot** (per VMM + profile): boot the erofs-rootfs base to "agent-ready," snapshot. Per-test = restore + tmpfs overlay. **(observed, v11) The Firecracker snapshot profile constrains the base image:** Firecracker restore can panic in `restore_fpregs_from_fpstate` when the guest `glibc` uses aggressive AVX/extended-FPU paths, so the implementer based the FC-snapshot rootfs on `debian:bookworm-slim` rather than the `trixie` default. This is a per-profile decision with a fidelity cost — three ways to resolve it (§15): (a) a Firecracker **static CPU template** (T2/C3-class) that masks the offending extended-state CPUID bits, keeping the `trixie` base and one shared erofs image (preferred); (b) `bookworm-slim` globally (one image, simplest, but tests on Debian 12 not 13); or (c) a separate `bookworm` FC-snapshot profile alongside a `trixie` CH image (preserves fidelity per backend but doubles erofs/snapshot artifacts and erodes the single-shared-erofs density win). CH and QEMU are unaffected.
...
**(observed, v11) First measurements — preliminary, fold in with caveats.** The first benchmark pass produced micro hot-path costs and a cross-backend cold-boot distribution; the full tables (with method notes) are in §15. Headlines: the micro hot-path is cheap as designed (postcard encode ≈53 ns / decode ≈89 ns, `cache_key` ≈198 ns, `/30` IPv4 parse ≈31 ns, in-memory empty-tar→erofs ≈1.27 µs). Cold boot to **agent-`Ready`** with host page-caches dropped (N=3) came in at **CH ≈293 ms p50, Firecracker ≈778 ms, QEMU ≈1125 ms** — which (a) **refutes the optimistic vendor figures** this section flagged as disputed (CH is *not* <100 ms; Firecracker is *not* ≈125 ms — that figure excludes console + agent-ready handshake + guest kernel/rootfs boot, which dominate, *not* the REST config the impl note blamed); and (b) **inverts the assumed ordering** — Firecracker is *not* faster than CH on this stack, so Firecracker's case rests on **density + snapshot**, not cold-boot latency. Two load-bearing caveats: **N=3 makes the percentiles degenerate** (p95=p99=max — distribution shape is not yet established; raise N before quoting tails), and **the restore numbers — the actual per-test hot path (§13.3) — are still missing**, so none of this yet measures the warm-start path the whole snapshot tier exists to make fast. Treat these as a first data point that settles the *cold-boot* dispute and the CH-vs-Firecracker *ordering*, not as the per-test budget.
...
- **Firecracker snapshot/restore now works, via the MMIO path §14.2 #1 proposed.** The guest kernel carries `CONFIG_VIRTIO_MMIO=y` and Firecracker runs its native MMIO mode (no `--enable-pci`), so it boots clean off the *same* `vmlinux` CH uses over PCI, and `snapshot_restore` flips `false→true`. This converts the v10 "degraded but honest" path into a real capability: `snapshot_restore.rs` now runs Firecracker (§12.4), and the §13.2 lazy-restore row is no longer CH-only. The validated restore sequencing differs from CH's and is now in §5.2/§5.3: pause/resume is `PATCH /vm` (not `PUT`); restore is a fresh process + `POST /snapshot/load {resume_vm:true}`; **drives and vsock may not be (re)configured around load**, so per-restore identity resolves relative snapshot paths per sandbox dir. (The one new caveat this surfaced is §15.2 #2.)
...
**2. Firecracker restore can panic on extended FPU/XSAVE state.** Restore can abort in `restore_fpregs_from_fpstate` when the guest userland exercises aggressive AVX/extended-FPU `glibc` paths (e.g. Ubuntu 24.04's libc): the saved XSAVE area mismatches the restore target. The implementer dodged it by basing the FC-snapshot rootfs on `debian:bookworm-slim` rather than the `trixie` default. **That works but trades guest-OS fidelity, and a more surgical fix exists (applied as a documented choice, §7 artifact 3 / §10 Exp 4):** a Firecracker **static CPU template** (T2/C3-class) masks the offending extended-state CPUID bits so the guest libc never selects those paths — keeping the `trixie` base and one shared erofs image. Three resolutions, in preference order: (a) CPU template + `trixie` (preserves fidelity and the single-shared-erofs density win); (b) `bookworm-slim` globally (simplest, one image, but the suite then tests Debian 12 not 13); (c) a separate `bookworm` FC-snapshot profile beside a `trixie` CH image (per-backend fidelity, but doubles erofs/snapshot artifacts). CH and QEMU place no such constraint.
...
| **Deviation:** the notes describe `AgentClient::exec`'s timeout as a **hardcoded 600 s**, used to cover long `apt-get` in the builder VM — but that reintroduces the v10 fast-fail regression for ordinary test execs (a hung exec now takes 10 min to fail) | The §5.2 design is a **per-request `timeout` on `ExecRequest`** — long (≈600 s) only for the builder-VM `mmdebstrap` call, short for tests, `None` = a bounded default. The 600 s ceiling belongs *on the builder-VM call site*, not on every `exec`. **Reconcile the implementation to per-request.** | §5.2 |

The first two are legitimate mechanism refinements (one config choice, one upstream-bug workaround). The third is the one place this pass appears to have **drifted from the design** rather than discovered something about the world: a single hardcoded timeout is easier to write than threading a per-request value, but it silently re-creates exactly the slow-fail problem the per-request design was added (in §14.3) to kill. Flagged here so the next pass closes it.

### 15.4 First measured numbers — preliminary, fold in with caveats

The first benchmark pass produced micro hot-path costs and a cross-backend **cold-boot** distribution. The hot-path numbers are stable (criterion, 100 samples); the macro numbers are **N=3 and cold-boot only** — read them as a first data point, not a settled result.

**Micro (criterion, 100 samples, in-process):**

| Benchmark | What it measures | p50 |
|---|---|---|
| `protocol_encode` | `postcard` length-delimited encode of `Message::Exec` | ≈53 ns |
| `protocol_decode` | `postcard` length-delimited decode | ≈89 ns |
| `cache_key_generation` | hashing struct variants + configs for the artifact cache key | ≈198 ns |
| `math_30_ipv4_parse` | `/30` host-IP parse (`10.200.<vmid>.1`) | ≈31 ns |
| `in_memory_tar2erofs_empty` | erofs node-tree pack of an empty tar stream, in-memory | ≈1.27 µs |
...
**Macro — cold boot to agent-`Ready`** (host page-caches dropped before each iteration; time from VMM boot command to the guest agent completing the vsock handshake and replying `Ready`; N=3):
...
Two results matter, and two caveats bound them. **Result 1 — the optimistic vendor figures are refuted (§3.4):** CH is *not* <100 ms, and Firecracker is *not* ≈125 ms. That ≈125 ms AWS figure is measured with the serial console disabled and a minimal kernel/rootfs, and excludes the agent-ready handshake — i.e. it excludes most of what this number includes. **Result 2 — the assumed ordering is inverted:** Firecracker is **not** faster than CH here (CH < Firecracker < QEMU). So Firecracker's case in this design rests on **density (low per-VM memory overhead) + snapshot**, *not* cold-boot latency — which is exactly why the per-test path is restore, not cold boot. **Caveat A — the impl note's attribution of Firecracker's 778 ms to "REST config API overhead" is a misdiagnosis:** a cold boot is dominated (~98%) by the guest kernel boot + agent startup, not the configuration API; Firecracker's multi-`PUT` config is on the order of a millisecond, and a reference snapshot *restore* is ≈tens of ms. The 778 ms is guest-boot-bound; chasing the REST path will not move it (snapshot/restore will). **Caveat B — N=3 makes the percentiles degenerate** (p95=p99=max; the distribution shape is not established). Raise N before quoting tails. **And the load-bearing gap: the restore numbers — the actual per-test hot path (§13.3) — are still missing.** Cold boot is the opt-in path (§4); nothing here yet measures the warm-start path the whole snapshot tier exists to make fast. Until `bench-vm` emits the restore distribution, the per-test budget is unmeasured.
...
| **Firecracker** + MMIO + built-in vsock + tap net | **Yes** (v11) | native MMIO snapshot; its vsock/balloon/rng/block are built-in, not vhost-user — *plus* the §15.2 #2 extended-FPU base-image guard |
...
*Version/feature claims reflect the mid-2026 research inputs; CH was at v52.0 and Kata 4.0 in preview at research time. Re-verify the §3 contested items — DAX availability, snapshot/virtio-fs composability, userfaultfd restore, nested-virt flags, and all boot/density numbers — against the exact tool versions pinned in `pins.lock`. §13 is the suite that performs that re-verification and settles the numbers; **as of v11 it has produced the first cold-boot pass (§15.4) — which already refutes the optimistic vendor cold-boot figures and inverts the assumed CH-vs-Firecracker ordering — but the restore distribution, the per-test hot path, is still outstanding.***
