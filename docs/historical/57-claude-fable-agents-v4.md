...
- Toolchain: `rust-toolchain.toml` pins 1.88 and the declared `rust-version` **equals** it (one
  `[workspace.package]` fact). An understated MSRV lets MSRV-aware resolvers hand consumers the
  vulnerable `time 0.3.45` the lockfile exists to avoid. Build `--locked`; never `cargo update` on
  an older toolchain.
