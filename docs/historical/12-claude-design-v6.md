...
**Experiment 4 — OCI-image rootfs (`oci-distribution` + `oci-spec`). Status: postponed.** *Goal:* pull a pinned Debian image by digest instead of assembling with `mmdebstrap`, eliminating the `dash`/`SHELL` quirk. *Reason deferred:* it would trade the Debian apt signing-chain verification (`InRelease`/`Release.gpg`, a stated requirement, satisfied by `mmdebstrap` + rPGP) for registry-digest trust. Kept on the shelf pending an explicit decision to accept that trade.
