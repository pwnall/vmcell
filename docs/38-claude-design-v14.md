...
One Cargo **package**, 2024 edition, with one **library crate** plus **binary** targets that wrap it. (**v14 forward-structural change:** the rebrand recommends promoting this single package to a **cargo workspace** — the `vmcell` library plus separate lean-binary member crates for the privileged-window / PID-1 binaries — so library churn no longer re-emits and re-strips the file caps of `vmcell-test-runner`; the rationale and the durable re-bless fix are in §12.8, the candidate entry is in §16.1.)
...
├─ src/bin/
│  ├─ vmcell.rs          # CLI wrapping the lib (clap): build, build-kernels, run, exec, ls, rm …
...
`vmcell-guest-agent` and `vmcell-test-runner` are deliberately thin. The agent shares only the small `agent::protocol` module with the host. The cap runner must be **blessed** once with privileges (file caps or setuid) and that blessing is stripped whenever the file is rewritten, so it must almost never rebuild: it depends only on `rustix` + `capctl`, pulls in no async runtime and **not** the `vmcell` library, and has no edge to `lib.rs`, so library churn never recompiles it. Keeping it tiny is also a security property — every dependency is code that runs inside the privileged window.
...
Maps onto the VM-artifact-production requirements: staged, pinned, deterministic, cacheable, resettable, minimal external access, record/replay, signing-chain verified. Exposed both as the library `artifact::Pipeline` API and as `vmcell build [--reset-to STAGE]` on the CLI.
...
- **Privileged suite (`just test-privileged`, via the §12.8 capability runner).** *Prerequisite probe (fail-loud, §7.1):* the suite confirms the **three caps in the *effective* set** (`CAP_NET_ADMIN`/`CAP_SYS_ADMIN`/`CAP_DAC_OVERRIDE`), `/var/run/netns` writability, and a **non-threaded `domain` cgroup scope** with the needed controllers delegated (§6.4/§7.2). It asserts the full-fidelity path:
  ...
  - *the gateway covers itself (partly today)*: the capability runner's **path confinement, `+ep` remediation message, and `kvm`-group-preservation logic are unit-tested**; extracting its privilege-drop sequence (inheritable → bounding-drop → ambient-raise → trim → uid) into a pure, unit-testable `CapState` transition is forward work (§12.8 #3), so today that ordering is exercised only by *running* the privileged suite, not by a unit test.
...
**Reducing re-bless churn — the maintainer's standing pain (v14, `todo.md` #2).** The dev loop is *rebuild → `just bless` → run*, and the re-bless step bites far more often than "only when the helper itself changes," because **writing the binary file strips its caps, and the file is rewritten by churn unrelated to the helper's own source**: switching `RUSTFLAGS` (a plain build vs `just ci`'s process-wide `RUSTFLAGS=-D warnings` re-fingerprints *every* target, the runner included), toggling feature sets (`--all-features` vs `--features test-runner`), and profile/toolchain changes all re-emit `target/<profile>/vmcell-test-runner` and drop its file caps. Three changes, in priority order, make blessing durable:

1. **Decouple the blessed artifact from `target/` (the load-bearing fix).** Build the runner once, **install a copy to a stable path** outside the cargo target tree — e.g. a gitignored `./.bin/vmcell-test-runner` or `$CARGO_HOME/bin` — and `setcap` *that copy*; point the nextest target-runner (`CARGO_TARGET_*_RUNNER`) at the stable path. Cargo's churn in `target/` then never touches the blessed binary, so re-bless is needed only when you deliberately reinstall (a real helper change). `just bless` becomes *build → install-to-stable → setcap stable*. This is robust to *all* unrelated rebuild churn — including the process-wide `RUSTFLAGS` re-fingerprint that a workspace alone cannot fix.
2. **Move the lean privileged-window binaries into cargo *workspace member crates* (structural; aligns with the rebrand).** Today vmcell is one package with the runner as a `[[bin]]` gated by `required-features`; the rebrand to a general runner makes a **workspace** the natural layout: a `vmcell` library crate plus member crates `vmcell-test-runner`, `vmcell-guest-agent`, and `vmcell-guest-tools`, each with its own `Cargo.toml`, dependency closure, and lint header. A member crate's fingerprint depends only on its own (tiny) source + deps, so **editing `lib.rs` or bumping a host-only dep never re-emits the runner** — removing the single largest source of incidental rebuilds and turning the §12.2 lean-tree assertion into a per-member property instead of a feature-gated whole-package check. (Members still share `target/` and `RUSTFLAGS`, so #1 stays the durable fix; #1 and #2 compose. This is the concrete answer to the todo's "should we switch to a cargo workspace?" — **yes**, primarily to isolate the privileged-window binaries' build fingerprints.)
3. **Test the privilege transition as pure logic so the runner rarely *needs* changing (the "more robust test suite" ask).** The runner already unit-tests its pure helpers — `blessing_remediation` (`+ep`, not `+p`), `confine_under` / `ensure_under_cargo_target_dir` (real path-descendant confinement, `..` rejected before canonicalization), and `merge_preserved_groups` (kvm-gid preserved iff held, never invented, never duplicated). The remaining untested sliver is `main()`'s capability-state sequence (inheritable-add → bounding-drop → ambient-raise → permitted/effective trim) and the uid drop, untested only because they mutate the live process. Extract that sequence into a **pure transition over an in-memory `CapState`** — compute the *next* `CapState` and the bounding-drop set from `(current, need, euid)` — and unit-test every step against its buggy inverse (a forgotten ambient-raise; a bounding set left wide; an inheritable not added; the uid-before-ambient ordering for the setuid form), leaving only the thin `set_current()`/`exec` syscalls as the integration-only edge. With the logic covered off the privileged path, runner edits become rare and are verified *before* a bless, so the privileged suite stops being the iteration loop for runner logic. (A `loom` or explicit state-machine test can additionally pin the ordering invariant.)
...
- **The cargo feature-matrix collapse + workspace split is forward structural work.** §10.5, §12.8, `todo.md` #2. Until the modules are re-gated onto the single `host` feature, the legacy `cargo hack` powerset gate stays red on partial-host-combo debt; the four-target build gate (§12.2) is the replacement. v14 additionally recommends extracting the three lean privileged-window binaries into **cargo workspace member crates** (§12.8) so library churn never re-emits — and re-strips the file caps of — the blessed `vmcell-test-runner`; combined with installing the blessed binary to a stable path outside `target/`, this is the concrete answer to the maintainer's re-bless-too-often pain.
...
- **Bring-your-own OCI image as an erofs rootfs source** (cross-cutting, V:high/E:med) — let a caller name any digest-pinned OCI image; the pipeline pulls by digest, verifies every blob, applies whiteouts, injects agent+CA+guest-tools, and packs a content-addressed erofs cached per digest. The on-ramp every competitor offers, without breaking the single-shared-erofs + snapshot story. *Builds on:* `artifact/rootfs/oci.rs` + the tar2erofs packer + the shared inject tail. *Guardrail:* digest-pinned only (provenance hard stop, §11.2), cache keyed on the erofs *output*; arbitrary images may omit the `libc6` the glibc agent needs → fall back to the static-musl agent and **fail loud** on incompatibility; keep rejecting OCI-as-runtime-overlay (snapshot/density); add the still-missing injectable OCI record/replay seam (§15).
...
- **Per-test kernel config-fragment matrix** (systems-testing, V:high/E:med) — extend the `kernels` registry so a test requests a kernel built from a base SHA + an overlay of KConfig fragments (KASAN, KCOV, lockdep, `slub_debug`, PREEMPT_RT, a driver), content-addressed per (base SHA + ordered fragment set); the matrix harness sweeps one test body across labels. The literal extension of v13's "kernel is a tracked dimension." *Builds on:* the multi-kernel registry + version-aware `artifact/kernel.rs`. *Guardrail:* hash the fragment set in sorted order, fold a stage-version + base SHA, validity content-addressed (§11.2); lives entirely in the pipeline, not the vmm/agent core; build-time blowup bounded by the cache.
...
- **Build & distribution hygiene: workspace split + content-hash bless stamp + reproducible bundle** (general, V:high/E:med — **answers `todo.md` #2**) — split the single package into a **cargo workspace** (lib crate + separate lean-binary crates: agent, test-runner/broker, guest-tools) so each has its own dependency graph and is stable across library churn; a `.blessed` stamp keyed on the binary's content hash so `just bless` re-runs only when the binary actually changed; and a reproducible distribution bundle (vendored pinned VMM binaries + kernels + erofs + CA, all digest-verified) so a consumer depends on one versioned artifact instead of the 7-step manual setup. *Builds on:* the four-target structure (§10.5) + the lean-tree CI assertions + `pins.json`. *Guardrail:* makes the lean-privileged-window boundary **structural** rather than feature-gated (stronger than the feature collapse); install verification fails on any digest mismatch (provenance hard stop); the redistributed-QEMU copyleft carve-out (external binary) must be re-checked for a *bundle*; see §12.8 for why this is the durable fix for the re-bless pain.
