# vmcell — Code Review Rubric (v3)

*Distilled from eight Claude review passes (docs 13, 17, 26, 27, 34, 37, 40, 46) and two Gemini
passes (25, 33) against the v6 → v18 design. Its job is to stop the **classes** of defect those
reviews found from recurring — not to re-list individual findings. This **v3 rubric** supersedes v2
(`docs/36-claude-code-review-rubric.md`), incorporating the three post-v2 passes (37, 40, 46).
Tagging: unmarked items carry over from v2 (reviews 13–34); **[37] [40] [46] [G]** mark items added
or sharpened by that pass (G = the Gemini passes); **[BP]** marks best practices added on judgment
for this problem domain, not yet matched to a surfaced defect.*
...
5. **Host-facing claims are validated by execution, not reading. [37]** A review of (or fix to)
   host-facing code is not done until the suites actually ran on a capable KVM host (Part E). Review
   37's empirical pass found two Highs and a leak that seventeen static sub-reviewers could not see.
...
// allows carrying "invariant:" comments — at minimum grep-ban `.expect(` in guest-driven modules.
```
...
- **Phase 0 preflight, block-and-ask.** A privileged-aware review starts by verifying the suites
  ...
  delegated scope available). If they can't, either fix that first or label the review
  **static-only** and mark every runtime claim unverified. Review 37's empirical pass found a
...
- **Ground in `implementation-notes.md` first.** Recorded, justified deviations are not
  ...
  empirically disproven (the "CH restore is a known gap" entry outlived the fix [37]).
...
- **A fix to host-facing code is not done** until the suites re-ran green on a KVM host (AGENTS.md
...
the bug lives), **faithful, failure-injectable fakes** (A9), **data-plane assertions** (A10), and
**gates that are reachable and can themselves fail** (Part D meta-rules) — all validated by
actually executing the suites (rule 5 + Part E).
