...
//
// Scope: this integration-level test asserts the reachable subset — VMM instance
// before cgroup — because `setup_env` builds a real `NetNamespace` via the concrete
// `RtNetlink` (Privileged net, needs CAP_NET_ADMIN), which an integration test
// cannot inject a recording seam into. The FULL teardown order (instance -> netns
// -> cgroup) is pinned by the in-crate unit tests
// `orchestrator::tests::test_drop_order_full_chain_{normal,on_panic}`, which
// construct `MicroVm` directly with a recording netns; the real-VM panic-residue
// test below covers actual netns cleanup.
#[tokio::test]
